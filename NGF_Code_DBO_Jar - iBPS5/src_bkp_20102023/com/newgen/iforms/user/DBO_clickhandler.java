		...
		try
		{
			if("GetGridRowCount".equalsIgnoreCase(control))
			{
				if("Q_USR_0_DBO_DECLINE_REJECT_DTLS".equalsIgnoreCase(stringdata)){
					...
					DBO.mLogger.debug(" gridRowCount " + gridRowCount);
					return gridRowCount;
				}
			...
			if("InternalClarification".equalsIgnoreCase(control))
			{
				...
				if(!output2.isEmpty()){
					for(int i =0; i<output2.size(); i++){
						...
						DBO.mLogger.debug(" JSONArray<Q_USR_0_DBO_INTERNAL_CLRFCTN_DTLS> output " + str_outputData);
						if(str_outputData.contains("IC_DEC_MAPID\":\"Y\"")){
							DBO.mLogger.debug("  This time IC_DEC_MAPID is Y " );
							str_arrRow += i;
						 }
					}
				}
				DBO.mLogger.debug(" str_arrRow " + str_arrRow);
				return str_arrRow;
			}
			//
			if("RelDedupeCifsSplit".equalsIgnoreCase(control))
			{
				...
				String selectedVal = (String) iformObj.getValue("Q_USR_0_DBO_RelatedPartyGrid_RCIF_ID");
 				DBO.mLogger.info("value selectedVal on load  ..." + selectedVal);
    			...
    			DBO.mLogger.debug(" DedupeCifs " + DedupeCifs);
    			if(!"".equalsIgnoreCase(DedupeCifs) && DedupeCifs != null)
    			{
	    			...
	    			for(int k = 0; k<arrDedupeCifs.length;k++)
	                {
		                ...
		                if(!"".equalsIgnoreCase(selectedVal) && arrDedupeCifs[k].equalsIgnoreCase(selectedVal)){
	 						DBO.mLogger.info("value selectedVal on load  ..." + selectedVal);
	 						DBO.mLogger.info("value valueCode on load  ..." + selectedVal);
	 						iformObj.setValue("Q_USR_0_DBO_RelatedPartyGrid_RCIF_ID", selectedVal);
	 					}
		                statusDedupeCifs = "inserted";
		                DBO.mLogger.debug(" inserted in combo " +  arrDedupeCifs[k]);
						DBO.mLogger.debug(statusDedupeCifs);
	                }
    			}
			...
			if("ClrGridData".equalsIgnoreCase(control))
			{
				String str_arrRow = "";
				try {
					DBO.mLogger.info("To populate Reasons on click of view History button   ...");
					String Query = "select DecisionSeqNo,ClarificationUniqueId,ClarificationCategory,ClarificationReason,"
							+ "ClarificationRemarks from USR_0_DBO_INTERNAL_CLRFCTN_DTLS  where"
							+ " WINAME = '" + WI_Name+ "'";
					DBO.mLogger.info("Query--" + Query);
					List<List<String>> lstVals= iformObj.getDataFromDB(Query);
					DBO.mLogger.info("Result  ..." + lstVals);
						if(lstVals != null){
							for (int j = 0; j < lstVals.size(); j++)
							{
								 str_arrRow += lstVals.get(j).get(0) + "#";
								 str_arrRow += lstVals.get(j).get(1) + "#";
								 str_arrRow += lstVals.get(j).get(2) + "#";
								 str_arrRow += lstVals.get(j).get(3) + "#";
								 if(j == lstVals.size() - 1){
									 str_arrRow += lstVals.get(j).get(4);
								 }else{
									 str_arrRow += lstVals.get(j).get(4) + "$";
								 }
							}
					...
					DBO.mLogger.debug("WINAME: " + WI_Name + ", WSNAME: " + activityName
						+ ", Exception in To populate Reasons on click of view History button: " + e.getMessage());
					}
				DBO.mLogger.debug(" str_arrRow " + str_arrRow);
				return str_arrRow;
			}
			...
			{
					DBO.mLogger.info("inside  PostHookloadSectionFirco ...");
					...
					DBO.mLogger.info("inside  PostHookloadSectionFirco after enable disable...");
					...
					for(int i=0; i<sizFircoGR; i++){
						...
						DBO.mLogger.debug(" value of isFrozen is " + isFrozen);
						if("Y".equalsIgnoreCase(isFrozen) || "Yes".equalsIgnoreCase(isFrozen) ){
							DBO.mLogger.debug("  This time isFrozen is Y " );
							str_arrRow += i;
						 }
					}	
					return str_arrRow;
			...
			{
					DBO.mLogger.info("inside  PostHookloadSectionPINGDATA ...");
					...
					DBO.mLogger.info("inside  PostHookloadSectionPINGDATA after enable disable...");
			...
			{
					DBO.mLogger.info("inside  PostHookloadSection ...");
					...
					DBO.mLogger.info("inside  PostHookloadSection after enable disable...");
					...
					if(!output3.isEmpty()){
						for(int i =0; i<output3.size(); i++){
							...
							String str_Decision = iformObj.getTableCellValue("Q_USR_0_DBO_WIHISTORY",i , 2);
							DBO.mLogger.debug(" JSONArray<Q_USR_0_DBO_WIHISTORY> output " + str_workstep);
							if(!("STP_Operator".equalsIgnoreCase(str_workstep) &&
									"Correction required at Front End".equalsIgnoreCase(str_Decision) ||
									"Assign To".equalsIgnoreCase(str_Decision))){
								DBO.mLogger.debug("  This time Workstep is not STP_Operator" );
								DBO.mLogger.debug("  This time Workstep is not STP_Operator and decision is not "
			...
			{ 
				...
				DBO.mLogger.info("To get limit value for Decline Grid ...");
				...
				DBO.mLogger.debug("DBO_DeclineReasonOneTimeLimit" + DBO_DeclineReasonOneTimeLimit);
				
				if(outputDeclineGR.size() < DBO_DeclineReasonOneTimeLimit){
					DBO.mLogger.debug("outputDeclineGR.size()" + outputDeclineGR.size() + "DBO_DeclineReasonOneTimeLimit" +DBO_DeclineReasonOneTimeLimit);
					return "GoodToGo";
			...
			{ 
				...
				// To get limit value ...
				try {
					DBO.mLogger.info("To get limit value  ...");
					String Query = "SELECT  CONST_FIELD_NAME,CONST_FIELD_VALUE FROM USR_0_BPM_CONSTANTS"
					...
					DBO.mLogger.debug("DBO_AddInfo_OneTimeLimit" + DBO_AddInfo_OneTimeLimit);
					DBO.mLogger.debug("DBO_AddInfo_LifeTimeLimit" + DBO_AddInfo_LifeTimeLimit);
					DBO.mLogger.debug("DBO_AddDoc_OneTimeLimit" + DBO_AddDoc_OneTimeLimit);
					DBO.mLogger.debug("DBO_AddDoc_LifeTimeLimit" + DBO_AddDoc_LifeTimeLimit);

					} catch (Exception e) {
					DBO.mLogger.debug("WINAME: " + WI_Name + ", WSNAME: " + activityName
						+ ", Exception in limit value load: " + e.getMessage());
					}
				...
				if(stringdata.equalsIgnoreCase("popupAddRowInfoGrid"))
				{
					...
					if(!outputData.isEmpty()){
						for(int i=0; i<outputData.size(); i++){
							...
							DBO.mLogger.debug(" ArrayList<Q_USR_0_DBO_ADDNAL_INFO_REQ_DTLS> outputData " + str_outputData);
					...
					DBO.mLogger.debug(" counterInfoGR " + counterInfoGR);
				
					DBO.mLogger.debug("inside control popupAddRowInfoGrid");
					if(output.size() >= DBO_AddInfo_LifeTimeLimit){
						DBO.mLogger.debug("output.size()" + output.size() + "DBO_AddInfo_LifeTimeLimit" +DBO_AddInfo_LifeTimeLimit);
						return "Life Time Maximum Limit Reached ! ";
					}else if(output.size() - counterInfoGR >= DBO_AddInfo_OneTimeLimit){
						DBO.mLogger.debug("counterInfoGR" + counterInfoGR + "DBO_AddInfo_OneTimeLimit" +DBO_AddInfo_OneTimeLimit);
						return "One Time Maximum Limit Reached ! ";
					}else{
						DBO.mLogger.debug("GoodToGo");
						return "GoodToGo";
					}
				}
				...
				{	
					...
					DBO.mLogger.debug(" counterDocGR " + counterDocGR);
					DBO.mLogger.debug("inside control popupAddRowDocGrid");
					if(output1.size() >= DBO_AddDoc_LifeTimeLimit){
						DBO.mLogger.debug("output1.size()" + output1.size() + "DBO_AddDoc_LifeTimeLimit" +DBO_AddDoc_LifeTimeLimit);
						return "Life Time Maximum Limit Reached ! ";
					}else if(output1.size() - counterDocGR >= DBO_AddDoc_OneTimeLimit){
						DBO.mLogger.debug("counterDocGR" + counterDocGR + "DBO_AddDoc_OneTimeLimit" +DBO_AddDoc_OneTimeLimit);
						return "One Time Maximum Limit Reached ! ";
					}else{
						DBO.mLogger.debug("GoodToGo");
						return "GoodToGo";
			...
			{ 
				...
				DBO.mLogger.info("onClick:::::::::: to check control for  userName 1  ");
				if("Q_USR_0_DBO_ADDNAL_INFO_REQ_DTLS".equalsIgnoreCase(tableId))
				{
					DBO.mLogger.info("onClick:::::::   to check control for  userName  2   ");
					...
					DBO.mLogger.info("onClick::::::: to check control for  userName  3   ");
				}
			...
			else if("OpenExpandedCommunication".equalsIgnoreCase(control))
			{
				...
				try 
				{
					DBO.mLogger.info("To get expanded data for internal clarification...");
				...
				{
					DBO.mLogger.debug("WINAME: " + WI_Name + ", WSNAME: " + activityName+ ", Exception in getting expanded internal clarification dtls: " + e.getMessage());
					return "";
				}
			} 
