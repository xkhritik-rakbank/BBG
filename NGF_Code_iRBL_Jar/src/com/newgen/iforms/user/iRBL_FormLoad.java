	...
	public String formLoadEvent(IFormReference iform, String controlName,String event, String data)
	{
        ...
        {	
        	...
        	try 
        	{
        		...
        		String query = "select count(*) from USR_0_IRBL_CountryMaster with(nolock) where countryCode in("
        				+ " (select NATIONALITY from USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS with(nolock) where wi_name = '"+getWorkitemName()+"' and NATIONALITY is not null and NATIONALITY != ''"
        				+ " UNION ALL"
        				+ " select ADDITIONALNATIONALITY from USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS with(nolock) where wi_name = '"+getWorkitemName()+"' and ADDITIONALNATIONALITY is not null and ADDITIONALNATIONALITY != ''"
        				+ " UNION ALL"
        				+ " select ADDITIONALNATIONALITY2 from USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS with(nolock) where wi_name = '"+getWorkitemName()+"' and ADDITIONALNATIONALITY2 is not null and ADDITIONALNATIONALITY2 != ''"
        				+ " UNION ALL"
        				+ " select ADDITIONALNATIONALITY3 from USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS with(nolock) where wi_name = '"+getWorkitemName()+"' and ADDITIONALNATIONALITY3 is not null and ADDITIONALNATIONALITY3 != '' )"
        				+ " )"
        				+ " and IsRestricted = 'Y'";
        ...
        {	
        	...
        	try 
        	{
        		...
        		String query = "select count(*) from USR_0_IRBL_CountryMaster with(nolock) where countryCode in("
        				+ " select DEMOGRAPHIC from USR_0_IRBL_DEMOGRAPHIC_DTLS with(nolock) where WI_NAME = '"+getWorkitemName()+"'"
        				+ " ) and IsDemographic = 'Y'";
    			List lstDecisions = iform
    				.getDataFromDB(query);
				iRBL.mLogger.debug("WINAME : "+getWorkitemName()+", WSNAME: "+iform.getActivityName()+", lstDecisions : "+lstDecisions.toString());
        ...
        else if("SetEmail".equals(controlName))
		{
			...
			{
				iRBL.mLogger.debug("WINAME : "+getWorkitemName()+", WSNAME: "+iform.getActivityName()+", Exception in loading Third Party Vendor Email " + e.getMessage());
			}
		}
        ...
        else if("ProposedTenorValue".equals(controlName))
        {
			...
			try {	
				...
				iRBL.mLogger.debug("WINAME : "+getWorkitemName()+", WSNAME: "+iform.getActivityName()+", Exception in loading Proposed Tenor value" + e.getMessage());
			}
		}
        else if("setProductPriorityLevelBasedOnPriority".equals(controlName))
        {
        	...
        	if("Express".equalsIgnoreCase(Priority.trim()))
        		query = "update WFINSTRUMENTTABLE set PriorityLevel = 4 where ProcessInstanceID = '"+getWorkitemName()+"'";
        	else 
        		query = "update WFINSTRUMENTTABLE set PriorityLevel = 1 where ProcessInstanceID = '"+getWorkitemName()+"'";
