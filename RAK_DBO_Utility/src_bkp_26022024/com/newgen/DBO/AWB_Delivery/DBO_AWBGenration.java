...
public class DBO_AWBGenration implements Runnable  {
	...
	public void run()
	{
		...
		try
		{
			...
			deleteSuccessDataBeforeDays=AWB_GEN_MAP.get("DeleteSuccessDataBeforeDays");
			DBO_AWB_Logs.DBO_AWBLogger.debug("DeleteSuccessDataBeforeDays: " + deleteSuccessDataBeforeDays);
		...
		try
		{
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
					...
					String extTabDataIPXML1 =CommonMethods.apSelectWithColumnNames(DBQuery1,cabinetName,CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false));
						...
						if(noOfRecords>0)
						{
							...
							for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
							{
								...
								if(!"Success".equalsIgnoreCase(AWBgenStatus))
								{
									...
									DoneWI( cabinetName, CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false), processInstanceID, WorkItemID, decision, Remarks,ActivityID,ActivityType,entryDateTime);
									break;
								}
	...
	private static String generateAWB (String WINAME,String RPID,HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			String AWBInputXML="<EE_EAI_MESSAGE>"+
					   ...
					   "</AWBGenerationRequest>"+
					"</EE_EAI_MESSAGE>";
			String integrationStatus = socketConnection(cabinetName, CommonConnection.getUsername(), CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false), jtsIP, jtsPort, WINAME, activityName, socketConnectionTimeout, integrationWaitTime,socketDetailsMap, AWBInputXML);
	...
	private static String updateTableData(String tablename, String columnname,String values, String sWhere)
	{
		...
		while(sessionCheckInt<loopCount)
		{
			...
			try
			{
				...
				String inputXmlcheckAPUpdate = CommonMethods.getAPUpdateIpXML(tablename,columnname,values,sWhere,cabinetName,CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false));
	...
	private static String generateSinglePager(String Product,String processInstanceID)
	{
		...
		try
		{
			...
			generatedPDFPath = objDocGen.generateAckTemplate(docToBeGen, templateSrcPath, singlePagerDestPath, processInstanceID, RelatedPartyData, CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false));
					...
					{	
						...
						if("S".equalsIgnoreCase(docAttachStatus))
						{
							...
							if(!("".equalsIgnoreCase(generatedPDFPath) || generatedPDFPath == null))
							{
								...
								String FinalPathofSinglepager=AWB_GEN_MAP.get("FinalSinglePagePath");
	 ...
	 private static String attachDocument(String filetobeaddedpath,String fileName,String workItemName)
	 {
	        try 
	        {
	            ...
	            if (!DocAttach.equalsIgnoreCase("N"))
	            {
	                ...
	                String sMappedInputXml = CommonMethods.getNGOAddDocument(parentFolderIndex,SinglePagerDocType,fileName,DocumentType,strExtension,sISIndex,sDocsize,VolumeID,cabinetName,CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false));
		...
		try
		{
			...
			DBO_AWB_Logs.DBO_AWBLogger.debug("SocketServerPort "+ socketServerPort);
	   		...
	   		if (!("".equalsIgnoreCase(socketServerIP) && socketServerIP == null && socketServerPort==0))
	   		{
    			...
    			inputRequest = getRequestXML( cabinetName,CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false) ,processInstanceID, ws_name, username, sInputXML);
    			...
    			if (num > 0)
    			{
    				...
    				if(!"".equalsIgnoreCase(outputResponse))
    					outputResponse = getResponseXML(cabinetName,sJtsIp,iJtsPort,CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false), processInstanceID,outputResponse,integrationWaitTime );
	...
	private static String getResponseXML(String cabinetName,String sJtsIp,String iJtsPort, String sessionId, String processInstanceID,String message_ID, int integrationWaitTime)
	{
		...
		try
		{
			...
			do
			{
				String responseInputXML =CommonMethods.apSelectWithColumnNames(QueryString, cabinetName, CommonConnection.getSessionID(DBO_AWB_Logs.DBO_AWBLogger, false));
	...
	private  void deleteOldFileFromSuccess()
	{
		try
		{
			File tobedeleted = new File(AWB_GEN_MAP.get("FinalSinglePagePath"));
			deleteFolder(tobedeleted);
			tobedeleted = new File(AWB_GEN_MAP.get("GeneratedSinglePager"));
			deleteFolder(tobedeleted);
		}
		catch(Exception e)
		{
			DBO_AWB_Logs.DBO_AWBLogger.error("Exception Occurred in deleteOldFileFromSuccess : "+e.toString());
		}
	}
	private static void deleteFolder(File file){
		...
		for (File subFile : file.listFiles()) 
		{
			...
			DBO_AWB_Logs.DBO_AWBLogger.info("File Name: "+subFile.getName()+", last modified: "+strModifiedDate);
			try {
			...
			if (isOld)
			{
				DBO_AWB_Logs.DBO_AWBLogger.info("Deleting: "+subFile.getName());	
