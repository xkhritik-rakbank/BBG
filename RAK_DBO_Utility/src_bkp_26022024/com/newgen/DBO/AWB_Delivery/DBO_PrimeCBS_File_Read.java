...
public class DBO_PrimeCBS_File_Read implements Runnable {
	...
	private static String deleteSuccessDataBeforeDays;
	...
	public void run()
	{
		...
		try
		{
			...
			deleteSuccessDataBeforeDays=DBOPrimeConfigParamMap.get("DeleteSuccessDataBeforeDays");
			DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.debug("deleteSuccessDataBeforeDays: "+deleteSuccessDataBeforeDays);
	...
	public String readDataFromPrimeFile(String finalSourcePath,String newFilename,String cabinetName, String sJtsIp, String iJtsPort)
	{
		...
		try
		{
			...
			while(read.hasNextLine())
			{
	    		//changed by gaurav
				s_tmp = read.nextLine();
				if("".equalsIgnoreCase(s_tmp) || s_tmp == null)
					continue;
				...
				if(col_val_ecrn.length<14)
				{
					return "fail";
				}
				elite_crn = col_val_ecrn[13];//till Prime 4 is going to be live
				str_Card_Type = col_val_ecrn[12];
				String CardSerNo = col_val_ecrn[11];
	    				
				DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.debug("ReadPrimeFile s_prime: "+s_tmp);
				...
				DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.debug("ReadPrimeFile CardSerNo: "+CardSerNo);

				// Ap select for getting Ecrn number check and insert flag
				String DebitCardRefNo="",Wi_name="", Query_for_ecrn="",RelatedPartyID="",ProspectID="",AccountNumber="",CustomerName="",MobNumber="",EmailID="",DebitCardRequired="",isChqBookRecipient="",ischqbkreq="",EmirateID="";
				Query_for_ecrn = "select TOP 1 r.DebitCardRefNo,e.WINAME,r.RelatedPartyID,e.ProspectID,e.AccountNumber,e.ischqbkreq,r.FullName,r.MobNumber,r.EmailID,r.DebitCardRequired,r.ChequeBookRecipient,r.EmirateID   from RB_DBO_EXTTABLE e with(nolock) , USR_0_DBO_RelatedPartyGrid r with(nolock) where e.WINAME=r.WINAME and  r.DebitCardRefNo='"+CardSerNo+"' and r.DebitCardRequired='Y' and (r.DebitCardBOFIleStatus != 'Received' or r.DebitCardBOFIleStatus is null)";
				String extTabDataIPXML=CommonMethods.apSelectWithColumnNames(Query_for_ecrn, cabinetName, CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false));
				...
				if("0".equalsIgnoreCase(mainCode))
				{
					...
					if(noOfRecords>0)
					{
						...
						for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
						{
							...
							EmirateID=objWorkList.getVal("EmirateID").trim();
							...
							status=updateInsertPRIMETable(DebitCardRefNo,s_prime_dbInsert,Wi_name,RelatedPartyID);
							if("Success".equalsIgnoreCase(status))
							{
								status=updateInsertAWBTable(DebitCardRefNo,Wi_name,RelatedPartyID,ProspectID,AccountNumber,CustomerName,MobNumber,EmailID,isChqBookRecipient,"PRIME",CardSerNo,EmirateID);
								if("Success".equalsIgnoreCase(status))
								{
	...
	public String ReadCSBEFile(String finalSourcePath,String newFilename,String cabinetName, String sJtsIp, String iJtsPort)
	{
		...
		try
		{
			...
			{
				tmp = read.nextLine();
				if("".equalsIgnoreCase(tmp) || tmp == null)
					continue;
				insert_val =tmp.replace("|", "#~#");
				insert_val_dbInsert="'"+tmp.replace("|", "','")+"'";
				DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.debug("input " + tmp);
			 	String [] chq_Bk_ref = insert_val.split("#~#");
			 	if(chq_Bk_ref.length<2)
			 	{
			 		return "fail";
			 	}
			 	String chqBk_ref = chq_Bk_ref[1];
			 	DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.debug("ReadCSBEFile : input row "+tmp);
			 	DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.debug("ReadCSBEFile : input chq_Bk_ref "+chq_Bk_ref[1]);
			 	
			 	String ChequeBk_ref="",WI_name="",RelatedPartyID="",ProspectID="",AccountNumber="",CustomerName="",MobNumber="",EmailID="",DebitCardRequired="",EmirateID="";
				...
				if("0".equalsIgnoreCase(mainCode))
				{
					...
					if(noOfRecords>0)
					{
						...
						for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
						{
							...
							if("Success".equalsIgnoreCase(status))
							{
								status=updateInsertAWBTable(chqBk_ref,WI_name,RelatedPartyID,ProspectID,AccountNumber,CustomerName,MobNumber,EmailID,DebitCardRequired,"CBS","",EmirateID);
								if("Success".equalsIgnoreCase(status))
								{
	...
	private String updateInsertPRIMETable(String dc_ref,String rowDataToinsertinDB,String winame, String RelatedPartyID)
	{
		String status="";
		try
		{
			String Query = "select count(*) as ResultRow from "+primeDataTableName+" with(nolock) where DebitCard_No='"+dc_ref+"'";
			String InputXML=CommonMethods.apSelectWithColumnNames(Query, cabinetName, CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false));
			...
			if("0".equalsIgnoreCase(mainCode))
			{
				...
				if(noOfRecords>0)
				{
					...
					if(intResultRowCount>0)
					{
						...
						String values ="getdate(),'"+RelatedPartyID+"','"+winame+"','"+newFilename+"',"+rowDataToinsertinDB;
						String sWhere ="DebitCard_No = '"+dc_ref+"'";
						status=updateTableData(primeDataTableName,"RowInsertUpdateDateTime,RelatedPartyID,"+primeCloumnName,values,sWhere);
					}
					else
					{
						String values ="'"+RelatedPartyID+"','"+winame+"','"+newFilename+"',"+rowDataToinsertinDB;
						status=insertDataInTable(primeDataTableName,"RelatedPartyID,"+primeCloumnName,values);
					}
				}
	...
	private String updateInsertAWBTable(String refNo,String winame,String RPID,String ProsID,String AccNo,String CustName,String MobNumber,String EmailID,String DebitCardorChequeBookReq,String updateFor, String CardSerNo, String EmirateID)
	{
		String status="";
		String Query ="";
		String columnName ="";
			...
			if("0".equalsIgnoreCase(mainCode))
			{
				...
				if(noOfRecords>0)
				{
					...
					if(intResultRowCount>0)
					{
						...
						{
							columnName="DebitCardRefNo,CardSerNo";
							values ="'"+refNo+"','"+CardSerNo+"'";
						}
					...
					else
					{
						if("CBS".equalsIgnoreCase(updateFor))
						{
							columnName="ProcessName,WI_name,RelatedPartyID,Prospect_ID,Account_no,Customer_name,mobile_No,email_id,ChequeBk_Req,ChequeBk_ref,card_req,EmiratesID";
							values ="'"+ProcessName+"','"+winame+"','"+RPID+"','"+ProsID+"','"+AccNo+"','"+CustName+"','"+MobNumber+"','"+EmailID+"','Y','"+refNo+"','"+DebitCardorChequeBookReq+"','"+EmirateID+"'";
						}
						else if("PRIME".equalsIgnoreCase(updateFor))
						{
							columnName="ProcessName,WI_name,RelatedPartyID,Prospect_ID,Account_no,Customer_name,mobile_No,email_id,ChequeBk_Req,DebitCardRefNo,CardSerNo,card_req,EmiratesID";
							values ="'"+ProcessName+"','"+winame+"','"+RPID+"','"+ProsID+"','"+AccNo+"','"+CustName+"','"+MobNumber+"','"+EmailID+"','"+DebitCardorChequeBookReq+"','"+refNo+"','"+CardSerNo+"','Y','"+EmirateID+"'";
						}
						else if("Declaration".equalsIgnoreCase(updateFor))
						{
							columnName="ProcessName,WI_name,RelatedPartyID,Prospect_ID,Account_no,Customer_name,mobile_No,email_id,ChequeBk_Req,card_req,EmiratesID";
							values ="'"+ProcessName+"','"+winame+"','"+RPID+"','"+ProsID+"','"+AccNo+"','"+CustName+"','"+MobNumber+"','"+EmailID+"','N','"+DebitCardorChequeBookReq+"','"+EmirateID+"'";
						}
	...
	private String updateTableData(String tablename, String columnname,String values, String sWhere)
	{
		...
		while(sessionCheckInt<loopCount)
		{
			...
			try
			{
				...
				String inputXmlcheckAPUpdate = CommonMethods.getAPUpdateIpXML(tablename,columnname,values,sWhere,cabinetName,CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false));
	...
	private String insertDataInTable(String tablename, String columnname,String values)
	{
		...
		while(sessionCheckInt<loopCount)
		{
			...
			{
				XMLParser objXMLParser = new XMLParser();
				String apInsertInputXML = CommonMethods.apInsert(cabinetName, CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false), columnname,values,tablename);
	...
	private void InsertDataInAWBForOnlyDeclaration(String WINAME)
	{
		try
		{
			String Query = "select r.RelatedPartyID,e.ProspectID,e.AccountNumber,e.ischqbkreq,r.FullName,r.MobNumber,r.EmailID,r.DebitCardRequired,r.ChequeBookRecipient,r.EmirateID   from RB_DBO_EXTTABLE e with(nolock) , USR_0_DBO_RelatedPartyGrid r with(nolock) "
			...
			String extTabDataIPXML=CommonMethods.apSelectWithColumnNames(Query, cabinetName, CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false));
			...
			XMLParser xmlParserData= new XMLParser(extTabDataOPXML);
			String mainCode = xmlParserData.getValueOf("MainCode");
			String RelatedPartyID="",ProspectID="",AccountNumber="",CustomerName="",MobNumber="",EmailID="",DebitCardRequired="",ischqbkreq="",EmirateID="";	
			if("0".equalsIgnoreCase(mainCode))
			{
				String recRetrived = xmlParserData.getValueOf("TotalRetrieved");
				int noOfRecords = (!"".equalsIgnoreCase(recRetrived) && recRetrived != null) ?Integer.parseInt(recRetrived):0;
				if(noOfRecords>0)
				{
					...
					for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
					{
						...
						ischqbkreq =objWorkList.getVal("ischqbkreq").trim();
						EmirateID =objWorkList.getVal("EmirateID").trim();
						String status=updateInsertAWBTable("",WINAME,RelatedPartyID,ProspectID,AccountNumber,CustomerName,MobNumber,EmailID,DebitCardRequired,"Declaration","",EmirateID);
						if("Success".equalsIgnoreCase(status))
						{
			...
			if("0".equalsIgnoreCase(mainCode))
			{
				...
				if(noOfRecords>0)
				{
					...
					if("0".equalsIgnoreCase(CBSFilePendingCount.trim()) && "0".equalsIgnoreCase(PRIMEFilePendingCount.trim()))
					{
						...
						//Lock Workitem	
						String WorkItemID="1";
						String getWorkItemInputXML = CommonMethods.getWorkItemInput(cabinetName, CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false), Wi_name,WorkItemID);
						...
						if ("0".equals(getWorkItemMainCode.trim()))
						{
							...
							String completeWorkItemInputXML = CommonMethods.assignWorkitemAttributeInput(cabinetName, CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false), Wi_name, WorkItemID,ActivityID,ActivityType, attributesTag,completeWIFlag);
							...
							if (completeWorkitemMaincode.trim().equalsIgnoreCase("0")) 
							{
								...
								String apInsertInputXML=CommonMethods.apInsert(cabinetName, CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false), columnNames, columnValues,"USR_0_DBO_WIHISTORY"); // toDo
								DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.debug("APInsertInputXML: "+apInsertInputXML);
		...
		while(sessionCheckInt<loopCount)
        {
            ...
            {	
				...
				sInputXML = "<?xml version=\"1.0\"?>" +
                        ...
                        "<SessionId>" + CommonConnection.getSessionID(DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger, false) + "</SessionId>" +
                        "</APInsert_Input>";
	...
	private  void deleteOldFileFromSuccess()
	{
		try
		{
			File tobedeleted = new File(primeFileSuccessPath);
			deleteFolder(tobedeleted);
			tobedeleted = new File(CBSFileSuccessPath);
			deleteFolder(tobedeleted);
		}
		catch(Exception e)
		{
			DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.error("Exception Occurred in deleteOldFileFromSuccess : "+e.toString());
		}
	}
	private static void deleteFolder(File file){
		...
		for (File subFile : file.listFiles()) 
		{
			...
			DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.info("File Name: "+subFile.getName()+", last modified: "+strModifiedDate);
			try {
			...
			if (isOld)
			{
				DBO_PrimeCBS_Logs.DBO_PrimeCBSLogger.info("Deleting: "+subFile.getName());	
