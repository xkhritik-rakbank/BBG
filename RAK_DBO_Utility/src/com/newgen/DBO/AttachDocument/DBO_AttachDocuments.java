...
public class DBO_AttachDocuments implements Runnable {
	...
	public void run()
	{
		...
		try
		{
			...
			deleteSuccessDataBeforeDays=DBODocumentCofigParamMap.get("DeleteSuccessDataBeforeDays");
			AttachDocLogs.DBO_AttachDocLogger.debug("DeleteSuccessDataBeforeDays: " + deleteSuccessDataBeforeDays);
												...
												for (int i = 0; i < 3; i++)
												{
													...
													if(lLngFileSize != 0L)
													{
														...
														try
														{
															...
															sMappedInputXml = CommonMethods.getNGOAddDocument(parentFolderIndex,DocNameAsProcess,strDocumentName,DocumentType,strExtension,sISIndex,lstrDocFileSize,volumeID,cabinetName,CommonConnection.getSessionID(AttachDocLogs.DBO_AttachDocLogger, false));
	...
	private void updateExternalTable(String tablename, String columnname,String sMessage, String sWhere)
	{
		...
		while(sessionCheckInt<loopCount)
		{
			try
			{
				...
				String inputXmlcheckAPUpdate = CommonMethods.getAPUpdateIpXML(tablename,columnname,sMessage,sWhere,cabinetName,CommonConnection.getSessionID(AttachDocLogs.DBO_AttachDocLogger, false));
		 ...
		 public static String get_timestamp()
				...
				while(sessionCheckInt<loopCount)
				{
					try{
						...
						if(workItemName!=null)
						{
							...
							String sInputXMLAPInsert = CommonMethods.apInsert(cabinetName,CommonConnection.getSessionID(AttachDocLogs.DBO_AttachDocLogger, false),columns,values,hist_table);
			...
			public String getEntryDatetimefromDB(String workItemName)
			{
				...
				while(sessionCheckInt<loopCount)
				{
					try {
						...
						String InputXMLEntryDate = CommonMethods.apSelectWithColumnNames(sqlQuery,cabinetName, CommonConnection.getSessionID(AttachDocLogs.DBO_AttachDocLogger, false));
			...
			private void doneWorkItem(String wi_name,String values,Boolean... compeletFlag)
			{
				...
				try
				{
					...
					while(sessionCheckInt<loopCount)
					{
						executeXMLMapMethod.put("getWorkItemInputXML",CommonMethods.getWorkItemInput(cabinetName,CommonConnection.getSessionID(AttachDocLogs.DBO_AttachDocLogger, false),wi_name, "1"));
					...
					if (CommonMethods.getTagValues((String)executeXMLMapMethod.get("getWorkItemOutputXML"),"MainCode").equalsIgnoreCase("0"))
					{
						...
						while(sessionCheckInt<loopCount)
						{
							executeXMLMapMethod.put("inputXml1",CommonMethods.completeWorkItemInput(cabinetName,CommonConnection.getSessionID(AttachDocLogs.DBO_AttachDocLogger, false),wi_name,Integer.toString(1)));
			...
			private  void deleteOldFileFromSuccess()
			{
				try
				{
					File tobedeleted = new File(destFilePath);
					deleteFolder(tobedeleted);
				}
				catch(Exception e)
				{
					AttachDocLogs.DBO_AttachDocLogger.error("Exception Occurred in deleteOldFileFromSuccess : "+e.toString());
				}
			}
			private static void deleteFolder(File file){
				...
				for (File subFile : file.listFiles()) 
				{
					...
					AttachDocLogs.DBO_AttachDocLogger.info("File Name: "+subFile.getName()+", last modified: "+strModifiedDate);
					try {
					...
					if (isOld)
					{
						AttachDocLogs.DBO_AttachDocLogger.info("Deleting: "+subFile.getName());	
