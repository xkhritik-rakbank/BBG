...
//	private static NGEjbClient ngEjbClientAWBGen;
	...
	private static String AccActivationBCCMailID = "";
...
//			ngEjbClientAWBGen = NGEjbClient.getSharedInstance();
			...
			AccActivationBCCMailID = NotifyAppConfigParamMap.get("AccActivationBCCMailID");
			DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("AccActivationBCCMailID: " + AccActivationBCCMailID);
...
//							String abc ="'"+ processInstanceID +" ', '', '" + NotifyDEHEvent + "', '', ' "+ DeliveryStatus +" ', ' "+ StatusCode +"', getDate(), " +"'"+ integrationStatus +"'";
		...
		try {
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0) 
			{
				for (int i = 0; i < fetchWorkitemListCount; i++)
				{
					...
					String extTabDataIPXML = CommonMethods.apSelectWithColumnNames(DBQuery,cabinetName,CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
					DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataIPXML: " + extTabDataIPXML);
					...
					if (xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec > 0)
					{
						...
						for (; objWorkList.hasMoreElements(true); objWorkList.skip(true)) 
						{
							...
							if("WI_STATUS".equals(NotifyDEHEvent))
							{
								...
								else if("AccuntClosure".equalsIgnoreCase(prevWSforNotify)||"CourierExpiry".equalsIgnoreCase(prevWSforNotify)||"AccountClosureCourierRTO".equalsIgnoreCase(prevWSforNotify)||"PostFactoExpiry".equalsIgnoreCase(prevWSforNotify)||"AccountClosurePostFacto".equalsIgnoreCase(prevWSforNotify))
								{
									...
									DBQuery = "select DeclineUniqueId,DeclineCategory,DeclineRemarks,DeclineType,DeclineMainCategory,declineCustomerMessage from USR_0_DBO_DECLINE_REJECT_DTLs with(nolock) where WINAME='"+ processInstanceID +"'";
									 extTabDataIPXML = CommonMethods.apSelectWithColumnNames(DBQuery,cabinetName,CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
									DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataIPXML: " + extTabDataIPXML);
	...
	public HashMap<String, String> getSocketConnectionDetails(String cabinetName, String sJtsIp, String iJtsPort, String sessionID) {
		...
		try {
			...
			String socketDetailsInputXML = CommonMethods.apSelectWithColumnNames(socketDetailsQuery, cabinetName, CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
	...
	private static void getCommunicationDetails()
	{
		try
		{
			String DBQuery = "select MailTemplate,MailPlaceHolders,mailSubject,fromMail,defaultCCmail,SMSEnglishTemplate from  USR_0_DBO_Communication_Templates with(nolock) where IsActiveMail='Y' or IsActiveEnglish='Y'";
			String extTabDataIPXML = CommonMethods.apSelectWithColumnNames(DBQuery,cabinetName,CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
			DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataIPXML: " + extTabDataIPXML);
			String extTabDataOPXML = CommonMethods.WFNGExecute(extTabDataIPXML,jtsIP,jtsPort, 1);
			DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataOPXML: " + extTabDataOPXML);
	...
	private String getResponseXML(String processInstanceID, String message_ID, int integrationWaitTime) {
		...
		try {
			...
			int Loop_count = 0;
			do {
				String responseInputXML = CommonMethods.apSelectWithColumnNames(QueryString, cabinetName, CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
	...
	private String getRequestXML(String processInstanceID, String ws_name, String userName, StringBuilder sInputXML) {
		...
		strBuff.append("<SessionId>" + CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false) + "</SessionId>");
	...
	private static String sendCommunication(String WINAME,String ProcessDefID, String activityId)
	{
		...
		try
		{
			String DBQuery = "select RP.relatedPartyID,RP.Fullname,RP.emailID,Concat(RP.MobNoCountryCode,RP.MobNumber) as MoBNO,EX.TLnumber,EX.AccountNumber,EX.ProductCurrency,EX.ProspectID from USR_0_DBO_RelatedPartyGrid RP with(nolock),RB_DBO_EXTTABLE EX with(nolock) where RP.winame=EX.winame and RP.winame='"+WINAME+"' and ( RP.DeclarationRequired='Y' or RP.DeclarationRequired='Yes')";
			String extTabDataIPXML = CommonMethods.apSelectWithColumnNames(DBQuery,cabinetName,CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
			DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataIPXML: " + extTabDataIPXML);
			String extTabDataOPXML = CommonMethods.WFNGExecute(extTabDataIPXML,jtsIP,jtsPort, 1);
			DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataOPXML: " + extTabDataOPXML);
			...
			if (xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec > 0)
			{
				...
				for (; objWorkList.hasMoreElements(true); objWorkList.skip(true)) 
				{
					...
					DBQuery = "select top 3 pd.ImageIndex,pd.Name,pd.VolumeId,ltrim(rtrim(pd.AppName)) as AppName,pd.CreatedDateTime,pd.comment "+
						          ...
						          "and (pd.Name = 'Signed Declaration form' or pd.Name = 'Account Opening Application' or pd.Name = 'Accepted Authorization') order by pd.CreatedDateTime desc";
					extTabDataIPXML = CommonMethods.apSelectWithColumnNames(DBQuery,cabinetName,CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
					DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataIPXML: " + extTabDataIPXML);
					extTabDataOPXML = CommonMethods.WFNGExecute(extTabDataIPXML,jtsIP,jtsPort, 1);
					DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("extTabDataOPXML: " + extTabDataOPXML);
					...
					if (xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec > 0)
					{
						...
						String mailSubject=commDetailsMap.get("mailSubject").replace("$ProspectId$",ProspectID );
						//String MailStr = commDetailsMap.get("MailTemplate").replace("$AccountNumber$",accNo).replace("$ProductCurrency$",productCurr).replace("$CustomerName$",Name);
						
						String AccountsAndCurrencies = "<tr>"
						...
						// Taking Multiple Currencies
						String DBQuery1 = "select AccountNumber,isnull(Currency,'') as Currency"
								+ " from USR_0_DBO_ADDNLCURRENCY_DTLS with(nolock) "
								+ " where WI_NAME = '"+WINAME+"' and AccountNumber is not null and AccountNumber !='' " ;
								
						String addnlAccountIPXML = CommonMethods.apSelectWithColumnNames(DBQuery1,CommonConnection.getCabinetName(), CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
						DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("Additional Account Data Input: "+ addnlAccountIPXML);
						...
						DBO_NotifyDEHLogs.DBO_NotifyLogger.debug("Additional Account Data output: "+ addnlAccountOPXML);

						XMLParser xmlParserData1 = new XMLParser(addnlAccountOPXML);						
						int iTotalrecAcc = Integer.parseInt(xmlParserData1.getValueOf("TotalRetrieved"));

						
						if(xmlParserData1.getValueOf("MainCode").equalsIgnoreCase("0")&& iTotalrecAcc>0)
						{
							...
							{	
									...
									String AddnlAccountNumber = objWorkList1.getVal("AccountNumber").trim();
									String AddnlCurrency = objWorkList1.getVal("Currency").trim();
									
									AccountsAndCurrencies = AccountsAndCurrencies + "<tr>"
											+ "	<td style=\"width:445px;text-align:center;\">"
												+ "	<p style = \"font-family:Verdana;font-size:11px;\">"
													+ AddnlAccountNumber
											+ "	</td>"
						...
						String MailStr = commDetailsMap.get("MailTemplate").replace("$CustomerName$",Name).replace("$AddnlAccountsAndCurrencies$", AccountsAndCurrencies);
	...
	public static String sendMail( String mailSubject,String MailStr,String mailTo,String ProcessDefID,String activityId,String attachmentISINDEX,String attachmentNames,String attachmentExts)throws Exception
	{
		...
		while(sessionCheckInt<loopCount)
		{
			try
			{
				...
				String columnName = "mailFrom,mailTo,mailCC,mailBCC,mailSubject,mailMessage,mailContentType,attachmentISINDEX,attachmentNames,attachmentExts,mailPriority,mailStatus,mailActionType,insertedTime,processDefId,workitemId,activityId,noOfTrials,zipFlag";
				String strValues = "'"+commDetailsMap.get("fromMail")+"','"+mailTo+"','"+commDetailsMap.get("defaultCCmail")+"','"+AccActivationBCCMailID+"','"+mailSubject+"',N'"+MailStr+"','text/html;charset=UTF-8','"+attachmentISINDEX+"','"+attachmentNames+"','"+attachmentExts+"','1','N','TRIGGER','"+CommonMethods.getdateCurrentDateInSQLFormat()+"','"+ProcessDefID+"','1','"+activityId+"','0','N'";
				...
				sInputXML = "<?xml version=\"1.0\"?>" +
						...
						"<SessionId>" + CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false) + "</SessionId>" +
						"</APInsert_Input>";
	...
	private static String updateTableData(String tablename, String columnname,String values, String sWhere)
	{
		...
		while(sessionCheckInt<loopCount)
		{
			...
			{
				XMLParser objXMLParser = new XMLParser();
				String inputXmlcheckAPUpdate = CommonMethods.getAPUpdateIpXML(tablename,columnname,values,sWhere,cabinetName,CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false));
	...
	private static String DoneWI (String processInstanceID,String WorkItemID, String decision, String Remarks,String ActivityID,String ActivityType,String entryDateTime)
	{
		...
		{
			//Lock WorkItem
			String getWorkItemInputXML = CommonMethods.getWorkItemInput(cabinetName, CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false), processInstanceID,WorkItemID);
			...
			if (getWorkItemMainCode.trim().equals("0"))
			{
				...
				String completeWorkItemInputXML = CommonMethods.assignWorkitemAttributeInput(cabinetName, CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false), processInstanceID, WorkItemID,ActivityID,ActivityType, attributesTag,completeWIFlag);
				...
				if (completeWorkitemMaincode.trim().equalsIgnoreCase("0")) 
				{
					...
					String apInsertInputXML=CommonMethods.apInsert(cabinetName, CommonConnection.getSessionID(DBO_NotifyDEHLogs.DBO_NotifyLogger, false), columnNames, columnValues,"USR_0_DBO_WIHISTORY"); // toDo
