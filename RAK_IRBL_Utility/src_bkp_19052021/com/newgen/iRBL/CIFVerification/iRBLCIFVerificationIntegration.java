		...
		try
		{
			...
			if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0")&& iTotalrec>0)
			{
				...
				{		
					...
					if((objWorkList.getVal("CIFVerificationStatus").equalsIgnoreCase("") || objWorkList.getVal("CIFVerificationStatus").equalsIgnoreCase("Failure")) && !objWorkList.getVal("CIF_ID").equalsIgnoreCase(""))
					{
						...
						String integrationStatus=CIFVerficationCall(CommonConnection.getCabinetName(),CommonConnection.getUsername(),CommonConnection.getSessionID(iRBLCIFVerificationLog.iRBLCIFVerificationLogger, false), CommonConnection.getJTSIP(),
								CommonConnection.getJTSPort(),processInstanceID,"",integrationWaitTime,socket_connection_timeout, socketDetailsMap, ExtTabDataMap, CheckGridDataMap);

						iRBLCIFVerificationLog.iRBLCIFVerificationLogger.debug("DedupeCall integrationStatus: " +integrationStatus);
