...
public class iRBLSysCheckIntegration implements Runnable{
	...
	{
		final String ws_name="SysCheckIntegration";
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
					...
					if (getWorkItemMainCode.trim().equals("0"))
					{
						...
						objResponseBean = IntegrateDedupeBlacklistFicroRiskscore.IntegratewithMW(processInstanceID, integrationWaitTime, socketConnectionTimeOut, socketDetailsMap);
						
						//objResponseBean= IntegrateInternalExposureCollectionsSummary.IntegratewithMW(integrationWaitTime, socketConnectionTimeOut, socketDetailsMap);					
						
						//objResponseBean= IntegrateExternalExposure.IntegratewithMW(integrationWaitTime, socketConnectionTimeOut, socketDetailsMap);					
						
						//objResponseBean= IntegrateFinancialSummary.IntegratewithMW(integrationWaitTime, socketConnectionTimeOut, socketDetailsMap);					
