...
public class iRBLIntegration {
			...
			HashMap<String, String> socketDetailsMap, HashMap<String, String> ExtTabDataMap, HashMap<String, String> RelPartyGridDataMap, boolean MainCIF_Flag)
	{
		try
		{
			...
			if(MainCIF_Flag)
			{
				...
				if (!"".equalsIgnoreCase(ExtTabDataMap.get("APPLICANT_FULL_NAME")))
					FullName="<FullName>"+ExtTabDataMap.get("APPLICANT_FULL_NAME")+"</FullName>";
			...
			else
			{
				...
				//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("else CIF_ID: "+CIF_ID);
				...
				//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("else NATIONALITY: "+NATIONALITY);

				DOB = RelPartyGridDataMap.get("DATEOFBIRTH");
				if(!DOB.trim().equalsIgnoreCase(""))
				{
					String tempDOB [] = DOB.split("-");
					DOB = tempDOB[0]+"-"+tempDOB[1]+"-"+tempDOB[2];
				}
				//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("else DOB: "+DOB);
				
				if (!"".equalsIgnoreCase(RelPartyGridDataMap.get("MIDDLENAME")))
					MidName="<MiddleName>"+RelPartyGridDataMap.get("MIDDLENAME")+"</MiddleName>";

				//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("else MidName: "+MidName);
				
				FullName = "<FullName>"+RelPartyGridDataMap.get("FIRSTNAME")+" "+RelPartyGridDataMap.get("LASTNAME")+"</FullName>";
				if (!"".equalsIgnoreCase(RelPartyGridDataMap.get("MIDDLENAME")))
					FullName="<FullName>"+RelPartyGridDataMap.get("FIRSTNAME")+" "+RelPartyGridDataMap.get("MIDDLENAME")+" "+RelPartyGridDataMap.get("LASTNAME")+"</FullName>";
				
				//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("else FullName: "+FullName);
			...
			StringBuilder sInputXML = new StringBuilder("<EE_EAI_MESSAGE>\n" +
					...
					"<CustomerDuplicationListRequest>" +
						"<BankId>RAK</BankId><CIFID>"+CIF_ID+"</CIFID><CustomerType>R</CustomerType>" +
						"<RetailCorpFlag>R</RetailCorpFlag><EntityType>All</EntityType>" +
						"<PersonDetails><FirstName>"+First_Name+"</FirstName>"+MidName+"" +
						"<LastName>"+Last_Name+"</LastName>"+FullName+"" +
			...
			{		    	
				...
				while(responseXML.contains("<Customer>"))
				{
					...
					String CustDormancy = (rowVal.contains("<CustDormancy>")) ? rowVal.substring(rowVal.indexOf("<CustDormancy>")+"</CustDormancy>".length()-1,rowVal.indexOf("</CustDormancy>")):"";
					/////////////////////////////////////////////
		...
		{
			iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Exception in DedupeCall Fn for WI :"+processInstanceID);
			return "Failure";
		}
	}
	...
	{	
		try
		{
			...
			{
				CIF_ID = ExtTabDataMap.get("CIF_NUMBER");
				if(!(ExtTabDataMap.get("ADDRESS_OR_EID").equals("")))
				{
					//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("ExtTabDataMap.get(ADDRESS_OR_EID) :"+ExtTabDataMap.get("ADDRESS_OR_EID"));
					
					DocDetXml = DocDetXml+"<Document>\n" +
						"<DocumentType>EMID</DocumentType>\n" +
						"<DocumentRefNumber>"+ExtTabDataMap.get("ADDRESS_OR_EID")+"</DocumentRefNumber>\n" +
					"</Document>";
				}
				/*if(!(ExtTabDataMap.get("PASSPORTNUMBER").equals("")))
				{
					DocDetXml = DocDetXml+"<Document>\n" +
						"<DocumentType>PPT</DocumentType>\n" +
						"<DocumentRefNumber>"+ExtTabDataMap.get("PASSPORTNUMBER")+"</DocumentRefNumber>\n" +
					"</Document>";
				}
				if(!(ExtTabDataMap.get("VISANUMBER").equals("")))
				{
					DocDetXml = DocDetXml+"<Document>\n" +
						"<DocumentType>VISA</DocumentType>\n" +
						"<DocumentRefNumber>"+ExtTabDataMap.get("VISANUMBER")+"</DocumentRefNumber>\n" +
					"</Document>";
				}*/
				
				//NATIONALITY = ExtTabDataMap.get("NATIONALITY");
				
				/*DOB = ExtTabDataMap.get("DATEOFBIRTH");
				if(!DOB.trim().equalsIgnoreCase(""))
				{
					String tempDOB [] = DOB.split("/");
				...
				//if (!"".equalsIgnoreCase(ExtTabDataMap.get("MIDDLENAME")))
					//MidName="<MiddleName>"+ExtTabDataMap.get("MIDDLENAME")+"</MiddleName>";
				
				//FullName = "<FullName>"+ExtTabDataMap.get("FIRSTNAME")+" "+ExtTabDataMap.get("LASTNAME")+"</FullName>";
				if (!"".equalsIgnoreCase(ExtTabDataMap.get("APPLICANT_FULL_NAME")))
					FullName="<FullName>"+ExtTabDataMap.get("APPLICANT_FULL_NAME")+"</FullName>";
			...
			StringBuilder sInputXML = new StringBuilder("<EE_EAI_MESSAGE>\n" +
					...
					"</EE_EAI_HEADER>" +
						"<CustomerBlackListRequest><BankId>RAK</BankId><CIFID>"+CIF_ID+"</CIFID>" +
						"<CustomerType>R</CustomerType><RetailCorpFlag>R</RetailCorpFlag>" +
						"<EntityType>All</EntityType><PersonDetails><FirstName>"+First_Name+"</FirstName>"+MidName+
						"<LastName>"+Last_Name+"</LastName>" +FullName+"" +
						"<MaritalStatus>"+Maritalstatus+"</MaritalStatus>" +
						"<Nationality>"+NATIONALITY+"</Nationality>" +
						"<DateOfBirth>"+DOB+"</DateOfBirth>" +
						"</PersonDetails>\n"+MobileNumberDetails+DocDetXml+"</CustomerBlackListRequest>\n" +
					"</EE_EAI_MESSAGE>");
		...
		{
			iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Exception in BlacklistCall Fn for WI: "+processInstanceID+", WSNAME: "+ws_name);
			return "Failure";
		}
	}
			...
			HashMap<String, String> socketDetailsMap, HashMap<String, String> ExtTabDataMap, HashMap<String, String> RelPartyGridDataMap, boolean MainCIF_Flag)
	{
		try
		{
			...
			else
			{
				...
				//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("else PASSPORT_NUMBER: "+PASSPORT_NUMBER);
		...
		StringBuilder sInputXML = new StringBuilder("<EE_EAI_MESSAGE>\n"+
								...
								"<RetryRequiredFlag>N</RetryRequiredFlag><RequestingUnitName>CENTRALOPERATIONSDUBAI</RequestingUnitName>" +
							...
							else if(arrOfStr1.length>2)
							{
								try {
		                            ...
		                            for(int i=1;i<arrOfStr1.length-1;i++)
									{
										...
										obj1.put("DATEOFBIRTH", Columnvalues.get("DATE OF BIRTH"));
									...
									//	obj1.put("REMARKS", Remarks);
										...
										iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("WINAME : "+processInstanceID+", WSNAME: "+ws_name+", @@@@@@@@@@ for fircosoft detail call 1 :::");
		...
		{
			iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Exception in FircosoftCall");
			return "Not Checked";
		}
	}
	...
	{	
		try
		{
			...
			String CustomerName=ExtTabDataMap.get("APPLICANT_FULL_NAME");
		...
		{
			iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Exception in RiskScore Call");
			return "Failure";
		}
	}
