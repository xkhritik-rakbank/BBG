...
public class iRBLBAISWICreate implements Runnable
{
	...
	public void run()
	{
		...
		try
		{
			...
			{
				iRBLBAISWICreateLog.iRBLBAISWICreateLogger.error("Could not Read Config Properties [RAOPStatus]");
				return;
			}
			...
			else
			{
				...
				{
					iRBLBAISWICreateLog.setLogger();
					iRBLBAISWICreateLog.iRBLBAISWICreateLogger.debug("RAOP Status...123.");
		...
		{
			e.printStackTrace();
			iRBLBAISWICreateLog.iRBLBAISWICreateLogger.error("Exception Occurred in RAOP CBS : "+e);
			final Writer result = new StringWriter();
			final PrintWriter printWriter = new PrintWriter(result);
			e.printStackTrace(printWriter);
			iRBLBAISWICreateLog.iRBLBAISWICreateLogger.error("Exception Occurred in RAOP CBS : "+result);
		}
	}
			...
			HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			iRBLBAISWICreateLog.iRBLBAISWICreateLogger.debug("Fetching all Workitems on RAOP_Status_Update queue");
			...
			iRBLBAISWICreateLog.iRBLBAISWICreateLogger.debug("Number of workitems retrieved on RAOP_Status_Update: "+fetchWorkitemListCount);
					...
					if (getWorkItemMainCode.trim().equals("0"))
					{
						...
						if(assignWorkitemAttributeMainCode.trim().equalsIgnoreCase("0"))
						{
							...
							if (completeWorkitemMaincode.trim().equalsIgnoreCase("0"))
							{
								...
								iRBLBAISWICreateLog.iRBLBAISWICreateLogger.debug("APInsertOutputXML: "+ apInsertInputXML);
