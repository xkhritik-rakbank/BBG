...
public class iRBLCIFUpdate implements Runnable
{
	...
	public void run()
	{
		...
		try
		{
			...
			else
			{
				...
				{
					iRBLCIFUpdateLog.setLogger();
					iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("RAOP Status...123.");
		...
		{
			e.printStackTrace();
			iRBLCIFUpdateLog.iRBLCIFUpdateLogger.error("Exception Occurred in RAOP CBS : "+e);
			final Writer result = new StringWriter();
			final PrintWriter printWriter = new PrintWriter(result);
			e.printStackTrace(printWriter);
			iRBLCIFUpdateLog.iRBLCIFUpdateLogger.error("Exception Occurred in RAOP CBS : "+result);
		}
	}
			...
			HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("Fetching all Workitems on RAOP_Status_Update queue");
			...
			iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("Number of workitems retrieved on RAOP_Status_Update: "+fetchWorkitemListCount);
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
					...
					iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("extTabDataIPXML: "+ extTabDataIPXML);
					String extTabDataOPXML = WFNGExecute(extTabDataIPXML,sJtsIp,iJtsPort,1);
					iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("extTabDataOPXML: "+ extTabDataOPXML);
					...
					//	String Dec = xmlParserExtTabDataRecord.getValueOf("DECISION");
						...
						{
							iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("Exception in getting reject code: "+e.getMessage());
						}
						...
						{
							iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("Exception in getting reject reason: "+e.getMessage());
						}
					...
					if (getWorkItemMainCode.trim().equals("0"))
					{
						...
						if(assignWorkitemAttributeMainCode.trim().equalsIgnoreCase("0"))
						{
							...
							if (completeWorkitemMaincode.trim().equalsIgnoreCase("0"))
							{
								...
								iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("Completed On "+ CurrWS);
			...
			{
				iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("WMFetchWorkList failed: "+fetchWorkItemListMainCode);
			}
