    ...
    public static String IntegratewithMW(String processInstanceID, int integrationWaitTime, int socketConnectionTimeOut, HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
        ...
        String DBQuery =
          "SELECT CIF_NUMBER AS 'CIF','Corporate_CIF'  AS 'CUSTOMER_TYPE',NEW_TOPUP AS 'Product',FINANCE_TYPE AS 'SubProduct','Primary' AS 'CIF_TYPE',INTERNAL_EXPOSURE_STATUS,EXTERNAL_EXPOSURE_STATUS, COLLECTION_SUMMARY_STATUS, FINANCIAL_SUMMARY_STATUS, CONVENTIONAL_ISLAMIC, TOTAL_ELIGIBILITY_AMOUNT, (select top 1 Description from USR_0_iRBL_Proposed_Tenor_Master with(nolock) where Code = PROPOSED_TENOR) AS PROPOSED_TENOR, 'EXT' as 'DATAFROM' FROM RB_iRBL_EXTTABLE WITH(NOLOCK) WHERE WINAME= '"
            + processInstanceID + "'"
            + "UNION ALL SELECT CIF AS 'CIF', CASE WHEN COMPANYFLAG='Y' THEN 'Corporate_CIF' WHEN COMPANYFLAG='YES' THEN 'Corporate_CIF' ELSE 'Individual_CIF' END  AS 'CUSTOMER_TYPE', '' AS 'Product','' AS 'SubProduct','Primary' AS 'CIF_TYPE',INTERNAL_EXPOSURE_STATUS,EXTERNAL_EXPOSURE_STATUS, COLLECTION_SUMMARY_STATUS, FINANCIAL_SUMMARY_STATUS, '' AS CONVENTIONAL_ISLAMIC, '' as TOTAL_ELIGIBILITY_AMOUNT, '' as PROPOSED_TENOR, 'RELATED' as 'DATAFROM' FROM USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS WITH(NOLOCK) WHERE WI_NAME='"
            + processInstanceID + "' ";
            ...
            for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
              {
                ...
                CheckGridDataMap.put("TOTAL_ELIGIBILITY_AMOUNT", objWorkList.getVal("TOTAL_ELIGIBILITY_AMOUNT"));
                ...
                {
                	TxmAmount = CheckGridDataMap.get("TOTAL_ELIGIBILITY_AMOUNT").trim();
                }
        ...
        if ("EXT".equalsIgnoreCase(CheckGridDataMap.get("DATAFROM")))
          {
            ...
            sInputXML = new StringBuilder("<EE_EAI_MESSAGE><EE_EAI_HEADER><MsgFormat>CUSTOMER_EXPOSURE</MsgFormat>"
              ...
              + "<CIFId><CIFIdType>" + CheckGridDataMap.get("CIF_TYPE") + "</CIFIdType>" + "<CIFIdValue>"
              + CheckGridDataMap.get("CIF") + "</CIFIdValue></CIFId><CustType>2</CustType><UserId>deepak</UserId>"
          ...
          {
            String BureauCategory = CheckGridDataMap.get("CUSTOMER_TYPE") == "Corporate_CIF" ? "Company" : "Retail";
            String DBQuery = "SELECT NAMEOFORGANIZATION AS 'COMPANY_NAME',TL_NUMBER, "
            		...
            		+ "GENDER, NATIONALITY "
            		+ "FROM USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS with(nolock) WHERE WI_NAME='"+wiName+"'  AND CIF = '"+CheckGridDataMap.get("CIF")+"'";
            String extTabDataIPXML =
              CommonMethods.apSelectWithColumnNames(DBQuery, CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false));
            iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("extTabDataIPXML: " + extTabDataIPXML);
            String extTabDataOPXML = iRBLSysCheckIntegration.WFNGExecute(extTabDataIPXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(), 1);
            ...
            sInputXML = new StringBuilder("<EE_EAI_MESSAGE><EE_EAI_HEADER><MsgFormat>CUSTOMER_EXPOSURE</MsgFormat>"
              ...
              + "<BranchId>RAK123</BranchId><RequestType>ExternalExposure</RequestType>"
              + "<CIFId><CIFIdType>" + CheckGridDataMap.get("CIF_TYPE") + "</CIFIdType>" + "<CIFIdValue>"
              + CheckGridDataMap.get("CIF") + "</CIFIdValue></CIFId>"+CustType+"<UserId>deepak</UserId>"
