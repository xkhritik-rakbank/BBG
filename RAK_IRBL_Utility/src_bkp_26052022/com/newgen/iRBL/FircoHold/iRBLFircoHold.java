		...
		try
		{
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
                    ...
                    {
						//As per part of production sanity -made changes 19032022
                        iRBLFircoHoldLog.iRBLFircoHoldLogger.debug("No records founds in Apselect "+xmlParserData1.getValueOf("MainCode")+" for the table : " +IRBL_FIRCO_GRID_DTLS+" with "+Totalrec1+" records");
                        decisionValue="NA";
					...
					{						
						...
						String DBQuery2 = "SELECT Firco_ID,Workstep_name,Call_type,StatusBehavior,StatusName,isnull(Remarks,'') as Remarks" +
						...
						if(xmlParserData2.getValueOf("MainCode").equalsIgnoreCase("0")&& Totalrec2>0)
						{
							...
							{	
								...
								if(StatusBehavior.equalsIgnoreCase("0000") || StatusBehavior.equalsIgnoreCase("FFF_OK") || StatusBehavior.equalsIgnoreCase("FFFBAD")) // updating firco status based on StatusBehavior as part jira POLP-10163
								{
									...
									//******************************************									
									String columnNames="MATCH_STATUS,REMARKS";
									String columnValues="'"+MatchStatusFlag+"','"+objWorkList.getVal("Remarks").trim()+"'";
