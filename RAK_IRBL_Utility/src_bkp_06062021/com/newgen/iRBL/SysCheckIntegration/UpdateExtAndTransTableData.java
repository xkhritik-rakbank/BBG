    ...
    public static String UpdateData(String processInstanceID) throws IOException, Exception
      {
        ...
        iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("inputXML: for external data update " + inputXML);
        outputXML = iRBLSysCheckIntegration.WFNGExecute(inputXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(), 1);
        iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("outputXML: for external data update: " + outputXML);
        xmlParserData = new XMLParser(outputXML);
        iTotalrec = Integer.parseInt(xmlParserData.getValueOf("TotalRetrieved"));
        ...
        if(!"".equalsIgnoreCase(AccOpenDate) || !"".equalsIgnoreCase(TotOutstandAmt))
        {
        	tableName = "RB_iRBL_EXTTABLE";
            columnNames = "ACCOUNT_OPENED_DATE";
            columnValues = "'" + AccOpenDate + "'";
        ...
        sQuery = "SELECT TOP 1 isnull(sum(CAST (AvgCrTurnOver AS INT)),0) AS AvgCrTurnOver,count(*) AS count FROM USR_0_iRBL_FinancialSummary_TxnSummary WITH(nolock) "+
        ...
        if (xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec > 0)
          {
            ...
            columnValues = "'" + xmlParserData.getValueOf("AvgCrTurnOver") + "','',''";
          }
