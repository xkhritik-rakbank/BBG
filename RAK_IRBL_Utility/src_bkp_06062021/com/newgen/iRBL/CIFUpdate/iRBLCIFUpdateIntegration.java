...
public class iRBLCIFUpdateIntegration
{
			...
			HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			String DBQuery = "SELECT 'Primary' AS CIFType, CIF_NUMBER AS CIFID, 'C' AS CustType, NTBEXISTING as NTBEXISTING, CIFUPDATEREQMAIN AS CIFUpdateStatus, PREFERREDADDRLINE1, PREFERREDADDRLINE2, PREFERREDADDRLINE3, PREFERREDADDRLINE4, PREFERREDCITY, PREFERREDCOUNTRYCODE, PREFERREDPOBOX, PREFERREDSTATE, PREFERREDZIPCODE, NUMBEROFYEARSINBUSINESS AS YEARSINBUSINESS, '' AS VISANUMBER, NULL AS VISAEXPIRYDATE  FROM RB_iRBL_EXTTABLE WITH(nolock) WHERE (CIFUPDATEREQMAIN = 'Y' OR CIFUPDATEREQMAIN = 'Yes') AND CIF_NUMBER IS NOT NULL AND CIF_NUMBER != '' AND WINAME = '"+processInstanceID+"'"+
					"UNION ALL "+
					"SELECT 'Related' AS CIFType, CIF AS CIFID, CASE WHEN COMPANYFLAG='Y' THEN 'C' WHEN COMPANYFLAG='YES' THEN 'C' ELSE 'R' END AS CustType, EXISTINGNTB as NTBEXISTING, CIFUPDATEREQREL AS CIFUpdateStatus, PREFERREDADDRLINE1, PREFERREDADDRLINE2, PREFERREDADDRLINE3, PREFERREDADDRLINE4, PREFERREDCITY, PREFERREDCOUNTRYCODE, PREFERREDPOBOX, PREFERREDSTATE, PREFERREDZIPCODE, '' AS YEARSINBUSINESS, VISANUMBER, isnull(format(VISAEXPIRYDATE,'yyyy-MM-dd'),NULL) AS VISAEXPIRYDATE FROM USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS WITH(nolock) WHERE (CIFUPDATEREQREL = 'Y' OR CIFUPDATEREQREL = 'Yes') AND CIF IS NOT NULL AND CIF != '' AND WI_NAME = '"+processInstanceID+"'";
			...
			iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("CIF Verification data input: "+ extTabDataIPXML);
			String extTabDataOPXML = CommonMethods.WFNGExecute(extTabDataIPXML,CommonConnection.getJTSIP(),CommonConnection.getJTSPort(),1);
			iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("CIF Verification data output: "+ extTabDataOPXML);
			...
			if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0")&& iTotalrec>0)
			{
				...
				{		
					...
					if((objWorkList.getVal("CIFUpdateStatus").equalsIgnoreCase("Y") || objWorkList.getVal("CIFUpdateStatus").equalsIgnoreCase("Yes")) && !objWorkList.getVal("CIFID").equalsIgnoreCase(""))
					{
						...
						String integrationStatus=CIFUpdateCall(CommonConnection.getCabinetName(),CommonConnection.getUsername(),CommonConnection.getSessionID(iRBLCIFUpdateLog.iRBLCIFUpdateLogger, false), CommonConnection.getJTSIP(),
								CommonConnection.getJTSPort(),processInstanceID,"",integrationWaitTime,socket_connection_timeout, socketDetailsMap, CheckGridDataMap);
