...
public class iRBLCIFUpdateIntegration
{
			...
			HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			String DBQuery = "SELECT 'Primary' AS CIFType, CIF_NUMBER AS CIFID, 'C' AS CustType, NTBEXISTING as NTBEXISTING, CIFUPDATEREQMAIN AS CIFUpdateStatus, PREFERREDADDRLINE1, PREFERREDADDRLINE2, PREFERREDADDRLINE3, PREFERREDADDRLINE4, PREFERREDCITY, PREFERREDCOUNTRYCODE, PREFERREDPOBOX, PREFERREDSTATE, PREFERREDZIPCODE, NUMBEROFYEARSINBUSINESS AS YEARSINBUSINESS, '' AS VISANUMBER, NULL AS VISAEXPIRYDATE,'' AS NAMEOFORGANIZATION, (select top 1 FinacleCode from USR_0_IRBL_INDUSTRYCODEMASTER with(nolock) where IndustryCode in (select top 1 INDUSTRY_CODE from USR_0_IRBL_INDUSTRY_CODE_DTLS with(nolock) where WI_NAME = '"+processInstanceID+"')) as INDUSTRY_CODE, (select FinacleCode from USR_0_IRBL_INDUSTRY_SUBCATEGORY_MASTER with(nolock) where INDUSTRY_SUBCATCODE in (select top 1 INDUSTRY_SUB_CATEGORY from USR_0_IRBL_INDUSTRY_CODE_DTLS with(nolock) where WI_NAME = '"+processInstanceID+"')) as INDUSTRY_SUB_CATEGORY, NUMBEROFYEARSINBUSINESS, (select top 1 isnull(AECBCONSENTSTATUSMAIN,'') from RB_iRBL_TXNTABLE with(nolock) where WI_NAME = '"+processInstanceID+"') as AECBCONSENTSTATUS, isnull(RISK_SCORE,'') as RISK_SCORE, '' as SIGNATORYPOWERHELDSINCEDATE, TL_NUMBER as TL_NUMBER, isnull(format(TL_VALID_TILL,'yyyy-MM-dd'),'') as TL_EXPIRY_DATE, '' as PASSPORTNUMBER, '' as PASSPORTEXPIRYDATE FROM RB_iRBL_EXTTABLE WITH(nolock) WHERE (CIFUPDATEREQMAIN is null OR CIFUPDATEREQMAIN != 'Success') AND CIF_NUMBER IS NOT NULL AND CIF_NUMBER != '' AND WINAME = '"+processInstanceID+"'"+
					"UNION ALL "+
					"SELECT 'Related' AS CIFType, CIF AS CIFID, CASE WHEN COMPANYFLAG='Y' THEN 'C' WHEN COMPANYFLAG='YES' THEN 'C' ELSE 'R' END AS CustType, EXISTINGNTB as NTBEXISTING, CIFUPDATEREQREL AS CIFUpdateStatus, PREFERREDADDRLINE1, PREFERREDADDRLINE2, PREFERREDADDRLINE3, PREFERREDADDRLINE4, PREFERREDCITY, PREFERREDCOUNTRYCODE, PREFERREDPOBOX, PREFERREDSTATE, PREFERREDZIPCODE, '' AS YEARSINBUSINESS, VISANUMBER, isnull(format(VISAEXPIRYDATE,'yyyy-MM-dd'),NULL) AS VISAEXPIRYDATE, '' AS NAMEOFORGANIZATION, (select top 1 FinacleCode from USR_0_IRBL_INDUSTRYCODEMASTER with(nolock) where IndustryCode in (select top 1 INDUSTRY_CODE from USR_0_IRBL_INDUSTRY_CODE_DTLS with(nolock) where WI_NAME = '"+processInstanceID+"')) as INDUSTRY_CODE, (select FinacleCode from USR_0_IRBL_INDUSTRY_SUBCATEGORY_MASTER with(nolock) where INDUSTRY_SUBCATCODE in (select top 1 INDUSTRY_SUB_CATEGORY from USR_0_IRBL_INDUSTRY_CODE_DTLS with(nolock) where WI_NAME = '"+processInstanceID+"')) as INDUSTRY_SUB_CATEGORY, '' as NUMBEROFYEARSINBUSINESS, isnull(AECBCONSENTSTATUS,'') as AECBCONSENTSTATUS, (select top 1 isnull(RISK_SCORE,'') from RB_iRBL_EXTTABLE with(nolock) where WINAME = '"+processInstanceID+"') as RISK_SCORE, isnull(format(SIGNATORYPOWERHELDSINCEDATE,'yyyy-MM-dd'),'') AS SIGNATORYPOWERHELDSINCEDATE, TL_NUMBER as TL_NUMBER, isnull(format(TLEXPIRYDATE,'yyyy-MM-dd'),'') as TL_EXPIRY_DATE, PASSPORTNUMBER, isnull(format(PASSPORTEXPIRYDATE,'yyyy-MM-dd'),'') as PASSPORTEXPIRYDATE FROM USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS WITH(nolock) WHERE (CIFUPDATEREQREL is null OR CIFUPDATEREQREL != 'Success') AND CIF IS NOT NULL AND CIF != '' AND WI_NAME = '"+processInstanceID+"'";
			...
			if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0")&& iTotalrec>0)
			{
				...
				{		
					...
					CheckGridDataMap.put("SIGNATORYPOWERHELDSINCEDATE", objWorkList.getVal("SIGNATORYPOWERHELDSINCEDATE"));
					CheckGridDataMap.put("TL_NUMBER", objWorkList.getVal("TL_NUMBER"));
					CheckGridDataMap.put("TL_EXPIRY_DATE", objWorkList.getVal("TL_EXPIRY_DATE"));
					CheckGridDataMap.put("PASSPORTNUMBER", objWorkList.getVal("PASSPORTNUMBER"));
					CheckGridDataMap.put("PASSPORTEXPIRYDATE", objWorkList.getVal("PASSPORTEXPIRYDATE"));
			...
			HashMap<String, String> socketDetailsMap, HashMap<String, String> CheckGridDataMap)
	{
		try
		{
			...
			if("R".equalsIgnoreCase(CustType))
			{
				...
				RetailAddnlDetails = RetailAddnlDetails + "<EmployeeStatus>Permanent</EmployeeStatus>\n";
				
				// changes done as part of mail subject - CIF Creation & Update changes
				if (!CheckGridDataMap.get("SIGNATORYPOWERHELDSINCEDATE").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("SIGNATORYPOWERHELDSINCEDATE").trim().equalsIgnoreCase("null"))
				{
					long diffInYears = CommonMethods.findDifference(CheckGridDataMap.get("SIGNATORYPOWERHELDSINCEDATE").trim(),"yyyy-MM-dd","Years");
					iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("SIGNATORYPOWERHELDSINCEDATE diffInYears: "+diffInYears);
					RetailAddnlDetails = RetailAddnlDetails + "<TotEmpYrs>"+diffInYears+"</TotEmpYrs>\n"; //Added by Suraj-28022022
					RetailAddnlDetails = RetailAddnlDetails + "<BusinessDuration>"+diffInYears+"</BusinessDuration>\n";
				}
				if (!CheckGridDataMap.get("AECBCONSENTSTATUS").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("AECBCONSENTSTATUS").trim().equalsIgnoreCase("null")) 
				{	
					String AECBConsentStatus= CheckGridDataMap.get("AECBCONSENTSTATUS").trim();
					if("Physical".equalsIgnoreCase(AECBConsentStatus) || "Digital".equalsIgnoreCase(AECBConsentStatus))
						AECBConsentStatus = "Y";
					else
						AECBConsentStatus = "N";
					
					RetailAddnlDetails = RetailAddnlDetails + "<AECBconsentHeld>"+AECBConsentStatus+"</AECBconsentHeld>\n";
				}
				if (!CheckGridDataMap.get("RISK_SCORE").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("RISK_SCORE").trim().equalsIgnoreCase("null")) 
				{	
					RetailAddnlDetails = RetailAddnlDetails + "<RiskProfile>"+CheckGridDataMap.get("RISK_SCORE").trim()+"</RiskProfile>\n";
				}
				/////////////////////////////////////////////////////////////
				...
				if (!CheckGridDataMap.get("VISANUMBER").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("VISANUMBER").trim().equalsIgnoreCase("null") && CheckGridDataMap.get("VISANUMBER").trim() != null && !CheckGridDataMap.get("VISAEXPIRYDATE").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("VISAEXPIRYDATE").trim().equalsIgnoreCase("null")  && CheckGridDataMap.get("VISAEXPIRYDATE").trim() != null)
				{
					DocDetails = "<DocDet>\n" +
				...
				// change done as part of mail subject "CIF Update Change" from Rachit on 15/06/2022
				if (!CheckGridDataMap.get("PASSPORTNUMBER").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("PASSPORTNUMBER").trim().equalsIgnoreCase("null") && CheckGridDataMap.get("PASSPORTNUMBER").trim() != null && !CheckGridDataMap.get("PASSPORTEXPIRYDATE").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("PASSPORTEXPIRYDATE").trim().equalsIgnoreCase("null")  && CheckGridDataMap.get("PASSPORTEXPIRYDATE").trim() != null)
				{
					DocDetails = DocDetails + "<DocDet>\n" +
						"<DocType>PPT</DocType>\n" +
						"<DocIsVerified>Y</DocIsVerified>\n"+
						"<DocNo>"+CheckGridDataMap.get("PASSPORTNUMBER").trim()+"</DocNo>\n" +
						"<DocExpDate>"+CheckGridDataMap.get("PASSPORTEXPIRYDATE").trim()+"</DocExpDate>\n" +
					"</DocDet>\n";
				}
			...
			if("C".equalsIgnoreCase(CustType))
			{
				...
				// change done as part of mail subject "CIF Update Change" from Rachit on 15/06/2022
				if (!CheckGridDataMap.get("TL_NUMBER").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("TL_NUMBER").trim().equalsIgnoreCase("null") && CheckGridDataMap.get("TL_NUMBER").trim() != null && !CheckGridDataMap.get("TL_EXPIRY_DATE").trim().equalsIgnoreCase("") && !CheckGridDataMap.get("TL_EXPIRY_DATE").trim().equalsIgnoreCase("null")  && CheckGridDataMap.get("TL_EXPIRY_DATE").trim() != null)
				{
					DocDetails = "<DocDet>\n" +
						"<DocType>TDLIC</DocType>\n" +
						"<DocIsVerified>Y</DocIsVerified>\n"+
						"<DocNo>"+CheckGridDataMap.get("TL_NUMBER").trim()+"</DocNo>\n" +
						"<DocExpDate>"+CheckGridDataMap.get("TL_EXPIRY_DATE").trim()+"</DocExpDate>\n" +
					"</DocDet>\n";
				}
