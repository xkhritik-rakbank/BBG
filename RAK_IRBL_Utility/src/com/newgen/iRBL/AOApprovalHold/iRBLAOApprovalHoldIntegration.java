...
public class iRBLAOApprovalHoldIntegration
{
	...
	{
		String ActivityName = "";
		String Dec_OPSDataEntryChecker="";
		...
		try
		{
			...
			if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0"))
			{
				...
				{		
					...
					String OFDBQuery = "select top 1 decision,actiondatetime,(select Current_WS from RB_BAIS_EXTTABLE with(nolock) where WI_NAME='"+BAISWIName+"') as CurrentWS from USR_0_BAIS_WIHISTORY with(nolock) where winame = '"+BAISWIName+"' and wsname = 'OPS_DataEntryChecker' order by id desc ";
					...
					if(xmlParserData1.getValueOf("MainCode").equalsIgnoreCase("0"))
					{
						...
						{		
							...
							Dec_OPSDataEntryChecker=objWorkList1.getVal("decision").trim();
							
							ActivityName=objWorkList1.getVal("CurrentWS").trim();

							if("Approve".equalsIgnoreCase(Dec_OPSDataEntryChecker.trim())) //Release the Wi from AO hold once WI approved from the ops dataentrychecker<RE: Items to be prioritized in IBPS-Rachit>
							{	
								ActivityName = "Exit";
								ActOpenDate = objWorkList1.getVal("actiondatetime").trim();
							}		
							ActivityName = ActOpenDate+"~"+ActivityName+"~"+BAISWIName;
