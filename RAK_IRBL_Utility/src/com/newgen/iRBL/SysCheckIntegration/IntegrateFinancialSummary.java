...
public class IntegrateFinancialSummary
  {
    ...
    public static String IntegratewithMW(String processInstanceID, int integrationWaitTime, int socketConnectionTimeOut, HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
        ...
        String DBQuery =
          "SELECT CIF_NUMBER AS 'CIF','Corporate_CIF'  AS 'CUSTOMER_TYPE',NEW_TOPUP AS 'Product',FINANCE_TYPE AS 'SubProduct','Primary' AS 'CIF_TYPE',INTERNAL_EXPOSURE_STATUS,EXTERNAL_EXPOSURE_STATUS, COLLECTION_SUMMARY_STATUS, FINANCIAL_SUMMARY_STATUS, CIF_NUMBER as 'RELATEDPARTYID' FROM RB_iRBL_EXTTABLE WITH(NOLOCK) WHERE WINAME= '"
            + processInstanceID + "'"
            + "UNION SELECT CIF AS 'CIF', CASE WHEN COMPANYFLAG='Y' THEN 'Corporate_CIF' WHEN COMPANYFLAG='YES' THEN 'Corporate_CIF' ELSE 'Individual_CIF' END  AS 'CUSTOMER_TYPE', '' AS 'Product','' AS 'SubProduct','Secondary' AS 'CIF_TYPE',INTERNAL_EXPOSURE_STATUS,EXTERNAL_EXPOSURE_STATUS, COLLECTION_SUMMARY_STATUS, FINANCIAL_SUMMARY_STATUS, RELATEDPARTYID FROM USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS WITH(NOLOCK) WHERE WI_NAME='"
            ...
            for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
              {
                ...
                if (!(CheckGridDataMap.get("FINANCIAL_SUMMARY_STATUS").equalsIgnoreCase("Y")))
                  {
                	String financialSummaryStatus = "";
                	if (!"".equalsIgnoreCase(CheckGridDataMap.get("CIF")) && CheckGridDataMap.get("CIF").length() == 7) 
                	{
                    	financialSummaryStatus = callFinancialSummary(processInstanceID, CheckGridDataMap, integrationWaitTime, socketConnectionTimeOut, socketDetailsMap);
                	} else 
                		financialSummaryStatus="Y";
                    ...
                    if ("Primary".equalsIgnoreCase(CheckGridDataMap.get("CIF_TYPE")))
                    ...
                    else
                      {
                        ...
                        columnNames = "FINANCIAL_SUMMARY_STATUS";
                        columnValues = "'" + financialSummaryStatus + "'";
                        sWhereClause = "WI_NAME='" + processInstanceID + "' AND RELATEDPARTYID='" + CheckGridDataMap.get("RELATEDPARTYID") + "'";
                      }
        ...
        try
          {
            ...
            String DBQuery =
              "SELECT ACCTID,AVGBALDET_STATUS, LIENDET_STATUS, RETURNDET_STATUS, SALDET_STATUS, SIDET_STATUS, TRANSUM_STATUS FROM USR_0_iRBL_InternalExpo_AcctDetails WITH(NOLOCK) WHERE AcctStat = 'ACTIVE' AND WI_NAME='" + wiName + "' AND CifId='" + CheckGridDataMap.get("CIF") + "'";
            String extTabDataIPXML =
              CommonMethods.apSelectWithColumnNames(DBQuery, CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false));
            iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("extTabDataIPXML: " + extTabDataIPXML);
            String extTabDataOPXML = iRBLSysCheckIntegration.WFNGExecute(extTabDataIPXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(), 1);
    ...
    private static String callAVGBALDET(String AcctID, String wiName, HashMap<String, String> CheckGridDataMap, int integrationWaitTime, int socketConnectionTimeOut,
      HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
        ...
        StringBuilder sInputXML = new StringBuilder("<EE_EAI_MESSAGE><EE_EAI_HEADER><MsgFormat>FINANCIAL_SUMMARY</MsgFormat>"
          ...
          + "<Extra1>REQ||LAXMANRET.LAXMANRET</Extra1><Extra2>"+DateExtra2+"</Extra2></EE_EAI_HEADER>" 
          + "<FinancialSummaryReq><BankId>RAK</BankId><CIFID>" + CheckGridDataMap.get("CIF")
          + "</CIFID><AcctId>" + AcctID + "</AcctId>" + "<OperationType>AVGBALDET</OperationType><NoOfMonths>11</NoOfMonths></FinancialSummaryReq></EE_EAI_MESSAGE>");
	    ...
	    String columnvalues="'"+wiName+"','"+CheckGridDataMap.get("CIF")+"','"+AcctID+"','FINANCIAL_SUMMARY','AVGBALDET','"+ReqDateTime+"','"+CallStatus+"','"+MsgId+"','"+ResDateTime+"','"+return_code+"','"+return_desc+"' ";
		...
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Input XML for apInsert FINANCIAL_SUMMARY(AVGBALDET) "+TableName+" Table : "+InputXML);

		String OutputXML=CommonMethods.WFNGExecute(InputXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(),1);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Output XML for FINANCIAL_SUMMARY(AVGBALDET) apInsert "+TableName+" Table : "+OutputXML);
    ...
    private static String callLIENDET(String AcctID, String wiName, HashMap<String, String> CheckGridDataMap, int integrationWaitTime, int socketConnectionTimeOut,
      HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
        ...
        StringBuilder sInputXML = new StringBuilder("<EE_EAI_MESSAGE><EE_EAI_HEADER><MsgFormat>FINANCIAL_SUMMARY</MsgFormat>"
          ...
          + "<RequestorLanguage>E</RequestorLanguage><RequestorSecurityInfo>secure</RequestorSecurityInfo>"
          + "<ReturnCode>911</ReturnCode><ReturnDesc>Issuer Timed Out</ReturnDesc><MessageId>143282719608815876</MessageId>"
          + "<Extra1>REQ||LAXMANRET.LAXMANRET</Extra1><Extra2>"+DateExtra2+"</Extra2></EE_EAI_HEADER>" + "<FinancialSummaryReq><BankId>RAK</BankId><CIFID>" + CheckGridDataMap.get("CIF")
		...
		if (responseXML.contains("<MessageId>"))
			MsgId = xmlParserSocketDetails.getValueOf("MessageId");
	    String CallStatus = "";
	    if(return_code.equals("0000") || return_code.equals("CINF729"))
	    	CallStatus="Success";
	    else
	    	CallStatus="Failure";
	    java.util.Date d2 = new Date();
	    ...
	    String TableName = "USR_0_IRBL_INTEGRATION_DTLS";
	    String columnnames="WI_NAME, CIFID, AccountNumber, CallName, Operation, RequestDateTime, CallStatus, MessageId, ResponseDateTime, ReturnCode, ReturnError";
	    String columnvalues="'"+wiName+"','"+CheckGridDataMap.get("CIF")+"','"+AcctID+"','FINANCIAL_SUMMARY','LIENDET','"+ReqDateTime+"','"+CallStatus+"','"+MsgId+"','"+ResDateTime+"','"+return_code+"','"+return_desc+"' ";
		String InputXML = CommonMethods.apInsert(CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false), columnnames, columnvalues, TableName);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Input XML for apInsert FINANCIAL_SUMMARY(LIENDET) "+TableName+" Table : "+InputXML);

		String OutputXML=CommonMethods.WFNGExecute(InputXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(),1);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Output XML for FINANCIAL_SUMMARY(LIENDET) apInsert "+TableName+" Table : "+OutputXML);
    ...
    private static String callRETURNDET(String AcctID, String wiName, HashMap<String, String> CheckGridDataMap, int integrationWaitTime, int socketConnectionTimeOut,
      HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
		...
		if (responseXML.contains("<MessageId>"))
			MsgId = xmlParserSocketDetails.getValueOf("MessageId");
	    String CallStatus = "";
	    if(return_code.equals("0000") || return_code.equals("CINF728"))
	    	CallStatus="Success";
	    else
	    	CallStatus="Failure";
	    java.util.Date d2 = new Date();
	    ...
	    String TableName = "USR_0_IRBL_INTEGRATION_DTLS";
	    String columnnames="WI_NAME, CIFID, AccountNumber, CallName, Operation, RequestDateTime, CallStatus, MessageId, ResponseDateTime, ReturnCode, ReturnError";
	    String columnvalues="'"+wiName+"','"+CheckGridDataMap.get("CIF")+"','"+AcctID+"','FINANCIAL_SUMMARY','RETURNDET','"+ReqDateTime+"','"+CallStatus+"','"+MsgId+"','"+ResDateTime+"','"+return_code+"','"+return_desc+"' ";
		String InputXML = CommonMethods.apInsert(CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false), columnnames, columnvalues, TableName);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Input XML for apInsert FINANCIAL_SUMMARY(RETURNDET) "+TableName+" Table : "+InputXML);

		String OutputXML=CommonMethods.WFNGExecute(InputXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(),1);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Output XML for FINANCIAL_SUMMARY(RETURNDET) apInsert "+TableName+" Table : "+OutputXML);
    ...
    private static String callSALDET(String AcctID, String wiName, HashMap<String, String> CheckGridDataMap, int integrationWaitTime, int socketConnectionTimeOut,
      HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
		...
		if (responseXML.contains("<MessageId>"))
			MsgId = xmlParserSocketDetails.getValueOf("MessageId");
	    String CallStatus = "";
	    if(return_code.equals("0000") || return_code.equals("CINF212"))
	    	CallStatus="Success";
	    else
	    	CallStatus="Failure";
	    java.util.Date d2 = new Date();
	    ...
	    String TableName = "USR_0_IRBL_INTEGRATION_DTLS";
	    String columnnames="WI_NAME, CIFID, AccountNumber, CallName, Operation, RequestDateTime, CallStatus, MessageId, ResponseDateTime, ReturnCode, ReturnError";
	    String columnvalues="'"+wiName+"','"+CheckGridDataMap.get("CIF")+"','"+AcctID+"','FINANCIAL_SUMMARY','SALDET','"+ReqDateTime+"','"+CallStatus+"','"+MsgId+"','"+ResDateTime+"','"+return_code+"','"+return_desc+"' ";
		String InputXML = CommonMethods.apInsert(CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false), columnnames, columnvalues, TableName);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Input XML for apInsert FINANCIAL_SUMMARY(SALDET) "+TableName+" Table : "+InputXML);

		String OutputXML=CommonMethods.WFNGExecute(InputXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(),1);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Output XML for FINANCIAL_SUMMARY(SALDET) apInsert "+TableName+" Table : "+OutputXML);
    ...
    private static String callSIDET(String AcctID, String wiName, HashMap<String, String> CheckGridDataMap, int integrationWaitTime, int socketConnectionTimeOut,
      HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
		...
		if (responseXML.contains("<MessageId>"))
			MsgId = xmlParserSocketDetails.getValueOf("MessageId");
	    String CallStatus = "";
	    if(return_code.equals("0000") || return_code.equals("CINF732"))
	    	CallStatus="Success";
	    else
	    	CallStatus="Failure";
	    java.util.Date d2 = new Date();
	    ...
	    String TableName = "USR_0_IRBL_INTEGRATION_DTLS";
	    String columnnames="WI_NAME, CIFID, AccountNumber, CallName, Operation, RequestDateTime, CallStatus, MessageId, ResponseDateTime, ReturnCode, ReturnError";
	    String columnvalues="'"+wiName+"','"+CheckGridDataMap.get("CIF")+"','"+AcctID+"','FINANCIAL_SUMMARY','SIDET','"+ReqDateTime+"','"+CallStatus+"','"+MsgId+"','"+ResDateTime+"','"+return_code+"','"+return_desc+"' ";
		String InputXML = CommonMethods.apInsert(CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false), columnnames, columnvalues, TableName);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Input XML for apInsert FINANCIAL_SUMMARY(SIDET) "+TableName+" Table : "+InputXML);

		String OutputXML=CommonMethods.WFNGExecute(InputXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(),1);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Output XML for FINANCIAL_SUMMARY(SIDET) apInsert "+TableName+" Table : "+OutputXML);
    ...
    private static String callTRANSUM(String AcctID, String wiName, HashMap<String, String> CheckGridDataMap, int integrationWaitTime, int socketConnectionTimeOut,
      HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
	    ...
	    String TableName = "USR_0_IRBL_INTEGRATION_DTLS";
	    String columnnames="WI_NAME, CIFID, AccountNumber, CallName, Operation, RequestDateTime, CallStatus, MessageId, ResponseDateTime, ReturnCode, ReturnError";
	    String columnvalues="'"+wiName+"','"+CheckGridDataMap.get("CIF")+"','"+AcctID+"','FINANCIAL_SUMMARY','TRANSUM','"+ReqDateTime+"','"+CallStatus+"','"+MsgId+"','"+ResDateTime+"','"+return_code+"','"+return_desc+"' ";
		String InputXML = CommonMethods.apInsert(CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false), columnnames, columnvalues, TableName);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Input XML for apInsert FINANCIAL_SUMMARY(TRANSUM) "+TableName+" Table : "+InputXML);

		String OutputXML=CommonMethods.WFNGExecute(InputXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(),1);
		iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Output XML for FINANCIAL_SUMMARY(TRANSUM) apInsert "+TableName+" Table : "+OutputXML);
