		...
		try
		{
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
					...
					iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("ActivityID: "+ActivityID);
					String ActivityType = xmlParserfetchWorkItemData.getValueOf("ActivityType");
					iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("ActivityType: "+ActivityType);
					String ProcessDefId = xmlParserfetchWorkItemData.getValueOf("RouteId");
					iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("ProcessDefId: "+ProcessDefId);
					...
					if (getWorkItemMainCode.trim().equals("0"))
					{
						...
						if("Success".equalsIgnoreCase(IntExpoCollSumStatus))
							MemopadStatus = IntegrateMemopadDetails.IntegratewithMW(processInstanceID,integrationWaitTime, socketConnectionTimeOut, socketDetailsMap);
						
						String updateStatus = "";
						if("Success".equalsIgnoreCase(IntExpoCollSumStatus) 
								&& "Success".equalsIgnoreCase(ExtExpoStatus) 
								&& "Success".equalsIgnoreCase(FinSumStatus))
							updateStatus = UpdateExtAndTransTableData.UpdateData(processInstanceID);
						
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status DedupeStatus: "+objResponseBean.getDedupeReturnCode());
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status BlacklistStatus: "+objResponseBean.getBlackListReturnCode());
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status FircoStatus: "+objResponseBean.getFircosoftReturnCode());
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status FircoHit: "+objResponseBean.getFircoHit());
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status IntExpoCollSumStatus: "+IntExpoCollSumStatus);
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status ExtExpoStatus: "+ExtExpoStatus);
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status FinSumStatus: "+FinSumStatus);
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status MemopadStatus: "+MemopadStatus);
						iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("Final Status updateStatus: "+updateStatus);
						
						String strIntegrationErrCode="";
						
						if("Record Found".equalsIgnoreCase(objResponseBean.getFircosoftReturnCode()) || "Record Not Found".equalsIgnoreCase(objResponseBean.getFircosoftReturnCode()))
							objResponseBean.setFircosoftReturnCode("Success");
						

						if("Success".equalsIgnoreCase(objResponseBean.getDedupeReturnCode()) 
								&& "Success".equalsIgnoreCase(objResponseBean.getBlackListReturnCode())
								&& "Success".equalsIgnoreCase(objResponseBean.getFircosoftReturnCode())
								&& "Success".equalsIgnoreCase(IntExpoCollSumStatus)
								&& "Success".equalsIgnoreCase(ExtExpoStatus)
								&& "Success".equalsIgnoreCase(FinSumStatus)
								&& "Success".equalsIgnoreCase(MemopadStatus))
						{
						...
						//String assignWorkitemAttributeInputXML=CommonMethods.assignWorkitemAttributeInput(CommonConnection.getCabinetName(), CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false), processInstanceID,WorkItemID,attributesTag);
						...
						String assignWorkitemAttributeInputXML = "<?xml version=\"1.0\"?><WMAssignWorkItemAttributes_Input>"
								...
								+ "<SessionId>"+CommonConnection.getSessionID(iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger, false)+"</SessionId>"
