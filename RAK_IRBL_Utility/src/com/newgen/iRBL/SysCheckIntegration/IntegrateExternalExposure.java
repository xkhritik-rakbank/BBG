...
public class IntegrateExternalExposure
  {
    ...
    public static String IntegratewithMW(String processInstanceID, int integrationWaitTime, int socketConnectionTimeOut, HashMap<String, String> socketDetailsMap) throws IOException, Exception
      {
        ...
        String DBQuery =
          "SELECT CIF_NUMBER AS 'CIF','Corporate_CIF'  AS 'CUSTOMER_TYPE',NEW_TOPUP AS 'Product',FINANCE_TYPE AS 'SubProduct','Primary' AS 'CIF_TYPE',INTERNAL_EXPOSURE_STATUS,EXTERNAL_EXPOSURE_STATUS, COLLECTION_SUMMARY_STATUS, FINANCIAL_SUMMARY_STATUS, CONVENTIONAL_ISLAMIC, REQUESTEDLOANAMOUNT, (select top 1 Description from USR_0_iRBL_Proposed_Tenor_Master with(nolock) where Code = PROPOSED_TENOR) AS PROPOSED_TENOR, 'EXT' as 'DATAFROM', COUNTRYOFINCORPORATION AS 'COUNTRYOFRESIDENCE', CIF_NUMBER AS 'RELATEDPARTYID', '' as 'RELATIONSHIPTYPE', '' as 'DATEOFBIRTH', '' as 'NATIONALITY', (select top 1 isnull(AECBCONSENTSTATUSMAIN,'') from RB_iRBL_TXNTABLE with(nolock) where WI_NAME = '"+processInstanceID+"') AS AECBCONSENTSTATUS FROM RB_iRBL_EXTTABLE WITH(NOLOCK) WHERE WINAME= '"
            + processInstanceID + "'"
            + "UNION ALL SELECT CIF AS 'CIF', CASE WHEN COMPANYFLAG='Y' THEN 'Corporate_CIF' WHEN COMPANYFLAG='YES' THEN 'Corporate_CIF' ELSE 'Individual_CIF' END  AS 'CUSTOMER_TYPE', '' AS 'Product','' AS 'SubProduct','Primary' AS 'CIF_TYPE',INTERNAL_EXPOSURE_STATUS,EXTERNAL_EXPOSURE_STATUS, COLLECTION_SUMMARY_STATUS, FINANCIAL_SUMMARY_STATUS, '' AS CONVENTIONAL_ISLAMIC, '' as REQUESTEDLOANAMOUNT, '' as PROPOSED_TENOR, 'RELATED' as 'DATAFROM',  CASE WHEN COMPANYFLAG='Y' THEN COUNTRY WHEN COMPANYFLAG='YES' THEN COUNTRY ELSE COUNTRYOFRESIDENCE END AS 'COUNTRYOFRESIDENCE', RELATEDPARTYID, RELATIONSHIPTYPE, isnull(format(DATEOFBIRTH,'dd-MM-yyyy'),'') as DATEOFBIRTH, isnull(NATIONALITY,''), isnull(AECBCONSENTSTATUS,'') FROM USR_0_IRBL_CONDUCT_REL_PARTY_GRID_DTLS WITH(NOLOCK) WHERE WI_NAME='"
            + processInstanceID + "' ";
            ...
            for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
              {
                ...
                CheckGridDataMap.put("AECBCONSENTSTATUS", objWorkList.getVal("AECBCONSENTSTATUS"));
                ...
                {	
                	...
                	{	
                		// 31012022 - Enquiry Purpose updated based on mail subject 'FW: 1365854 [RAKBANK-Internal]' from Amardeep
                ...
                if("".equalsIgnoreCase(mainCIFID) 
                		&& "EXT".equalsIgnoreCase(CheckGridDataMap.get("DATAFROM").trim()))
                {
                	mainCIFID = CheckGridDataMap.get("CIF").trim();
                }
                ...
                // As per sameera mail subject (RE: jira 10102) - POLP-10764
                if(!"EXT".equalsIgnoreCase(CheckGridDataMap.get("DATAFROM").trim()))
                {
                	String DateOfBirth = objWorkList.getVal("DATEOFBIRTH").trim();
                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("DateOfBirth- "+DateOfBirth);
                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("COUNTRYOFRESIDENCE- "+objWorkList.getVal("COUNTRYOFRESIDENCE").trim());
                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("RELATIONSHIPTYPE- "+objWorkList.getVal("RELATIONSHIPTYPE").trim());
                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("CUSTOMER_TYPE- "+objWorkList.getVal("CUSTOMER_TYPE").trim());
                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("NATIONALITY- "+objWorkList.getVal("NATIONALITY").trim());
                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("AECBCONSENTSTATUS- "+objWorkList.getVal("AECBCONSENTSTATUS").trim());
                	//iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("");
                	
                	if("Not held".equalsIgnoreCase(objWorkList.getVal("AECBCONSENTSTATUS").trim()))
                	{
                		CheckGridDataMap.put("EXTERNAL_EXPOSURE_STATUS", "NA");
	                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("For AECB Consent Not Held External Exposure call will not be called");
                	}
                	else if(!"AE".equalsIgnoreCase(objWorkList.getVal("COUNTRYOFRESIDENCE").trim()))
	                {
	                	CheckGridDataMap.put("EXTERNAL_EXPOSURE_STATUS", "NA");
                	...
                	else if("Individual_CIF".equalsIgnoreCase(objWorkList.getVal("CUSTOMER_TYPE").trim()) 
                			&& "AE".equalsIgnoreCase(objWorkList.getVal("NATIONALITY").trim())
                			&& !"Signatory".equalsIgnoreCase(objWorkList.getVal("RELATIONSHIPTYPE").trim())
                			&& !"Guarantor".equalsIgnoreCase(objWorkList.getVal("RELATIONSHIPTYPE").trim())
                			&& !("Borrower".equalsIgnoreCase(objWorkList.getVal("RELATIONSHIPTYPE").trim()) || "Co Borrower".equalsIgnoreCase(objWorkList.getVal("RELATIONSHIPTYPE").trim())))
                	{
                		CheckGridDataMap.put("EXTERNAL_EXPOSURE_STATUS", "NA");
	                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("For Individual_CIF, AE Nationality, Non Signatory, Non guarantor -   External Exposure call will not be called");
                	}
                	else if("Corporate_CIF".equalsIgnoreCase(objWorkList.getVal("CUSTOMER_TYPE").trim()) 
                			&& "AE".equalsIgnoreCase(objWorkList.getVal("COUNTRYOFRESIDENCE").trim())
                			&& !"Guarantor".equalsIgnoreCase(objWorkList.getVal("RELATIONSHIPTYPE").trim())
                			&& !("Borrower".equalsIgnoreCase(objWorkList.getVal("RELATIONSHIPTYPE").trim()) || "Co Borrower".equalsIgnoreCase(objWorkList.getVal("RELATIONSHIPTYPE").trim())))
                	{
                		CheckGridDataMap.put("EXTERNAL_EXPOSURE_STATUS", "NA");
	                	iRBLSysCheckIntegrationLog.iRBLSysCheckIntegrationLogger.debug("For Individual_CIF, AE Country of Incorporation, Non guarantor -   External Exposure call will not be called");
                	}
                ...
                {
                    String flag = callExternalExposure(processInstanceID, CheckGridDataMap, integrationWaitTime, socketConnectionTimeOut, socketDetailsMap, InquiryPurpose, TxmAmount, ProposedTenor, mainCIFID);
    ...
    private static String callExternalExposure(String wiName, HashMap<String, String> CheckGridDataMap, int integrationWaitTime, int socketConnectionTimeOut, HashMap<String, String> socketDetailsMap, String InquiryPurpose, String TxmAmount, String ProposedTenor, String mainCIFID)
	        ...
	        if ("EXT".equalsIgnoreCase(CheckGridDataMap.get("DATAFROM")))
	          {
	            ...
	            String mobileNumber = xmlParser.getValueOf("MOBILE");
	            mobileNumber = mobileNumber.replace("(", "");
	            mobileNumber = mobileNumber.replace(")", "");
	            mobileNumber = mobileNumber.replace(" ", "");
	            
	            /* 22122023 - as part of mailsubject "REQUEST FOR REMOVE AECB REQUEST" from Sameera below changes are done:
	             	Default OverridePeriod flag to 0 for all the AECB calls that gets triggered in both company and individual calls.
					LosIndicator to be defaulted to 4 means source system will be ibps.
					Primary mobile number in retail aecb call to be sent without spaces in between as below.
	            */
	            ...
	            sInputXML = new StringBuilder("<EE_EAI_MESSAGE><EE_EAI_HEADER><MsgFormat>CUSTOMER_EXPOSURE</MsgFormat>"
	              ...
	              + "<LosIndicator>4</LosIndicator><ContractType>1</ContractType><OverridePeriod>0</OverridePeriod>" 
	        ...
	        else
	          {
	            ...
	            String DBQuery = "SELECT NAME_OF_SISTER_COMPANY AS 'COMPANY_NAME',TL_NUMBER, "
	            		...
	            		+ "GENDER, NATIONALITY, (SELECT top 1 isnull(CBApplicationID,'') FROM USR_0_iRBL_InternalExpo_Derived with(nolock) WHERE Wi_Name='"+wiName+"' AND CifId = '"+mainCIFID+"' and Request_Type = 'ExternalExposure') as CBApplicationID "
	            ...
	            if("Retail".equalsIgnoreCase(BureauCategory))
	            {
	            	...
	            	{
	            		String EMID = xmlParser.getValueOf("EMIRATESID").trim();
	            		if (EMID.length() == 15)
	            			EMID = EMID.substring(0, 3)+"-"+EMID.substring(3, 7)+"-"+EMID.substring(7, 14)+"-"+EMID.substring(14, 15); // POLP-11293
	            ...
	            // Contract Type passed based on mail subject ' Role Types - Meeting Action Point Neha'
	            String ContractType = "3"; // 3 - Signatory/Guarantor/ Shareholder
	            if ("Borrower".equalsIgnoreCase(CheckGridDataMap.get("RELATIONSHIPTYPE").trim()) || "Co Borrower".equalsIgnoreCase(CheckGridDataMap.get("RELATIONSHIPTYPE").trim()))
	            	ContractType = "2"; // 2 - Co-borrrower 
	            
	            String CBApplicationID = xmlParser.getValueOf("CBApplicationID").trim();
	            ...
	            sInputXML = new StringBuilder("<EE_EAI_MESSAGE><EE_EAI_HEADER><MsgFormat>CUSTOMER_EXPOSURE</MsgFormat>"
	              ...
	              //+ "<ProviderApplNo>" + wiName.split("-")[1] + (new Date()).getTime() + "</ProviderApplNo>" // not to be passed for contract type 2 or 3
	              + "<CBApplNo>"+CBApplicationID+"</CBApplNo>"
	              + "<IsCoApplicant>1</IsCoApplicant>"
	              + "<LosIndicator>4</LosIndicator><ContractType>"+ContractType+"</ContractType><OverridePeriod>0</OverridePeriod>" //Added by suraj-Rachit01032022
	              + "<PrimaryMobileNo>" + mobileNumber
