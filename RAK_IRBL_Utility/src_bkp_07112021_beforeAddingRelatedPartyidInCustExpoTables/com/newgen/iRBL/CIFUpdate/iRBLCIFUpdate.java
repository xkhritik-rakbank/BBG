		...
		try
		{
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
					...
					iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("ActivityID: "+ActivityID);
					String ActivityType = xmlParserfetchWorkItemData.getValueOf("ActivityType");
					iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("ActivityType: "+ActivityType);
					String ProcessDefId = xmlParserfetchWorkItemData.getValueOf("RouteId");
					iRBLCIFUpdateLog.iRBLCIFUpdateLogger.debug("ProcessDefId: "+ProcessDefId);
