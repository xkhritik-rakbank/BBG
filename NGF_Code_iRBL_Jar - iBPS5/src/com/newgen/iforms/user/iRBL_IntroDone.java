...
public class iRBL_IntroDone extends iRBL_Common
{
	public String onIntroduceDone(IFormReference iform, String controlName,String event, String data)
	{
		...
		if("InsertIntoHistory".equals(controlName))
		{
			try {
				...
				for(int p=0;p<Integer.parseInt(data);p++)
				{
					...
					iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Complete Reject Reasons" + completeReason);
					...
					{						
						if(completeReason.indexOf("-")>-1)
						{
							...
							iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons code" + strRejectCodes);
							strRejectReasons=completeReason.substring(completeReason.indexOf("-")+1);
							iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons" + strRejectReasons);
						}
						else
						{
							strRejectReasons=completeReason;
							iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons else block" + strRejectReasons);
						}
					}	
					else
					{
						iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons 1" + strRejectReasons);						
						if(completeReason.indexOf("-")>-1)
						{
						...
						else
						{
							...
							iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons else block2" + strRejectReasons);
						}
						
						iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons 2" + strRejectReasons);
					}
			   ...
			   // SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");			   
				...
				iRBL.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", WI Histroy Added Successfully!");
			} 
			catch (Exception e) {
		...
		if("SystemCheckIntegrationlatestSuccessDate".equals(controlName))  // this is required to check 30 days expiry on the queues
		{
			...
			{
				iRBL.mLogger.debug("WINAME : "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", Exception in SystemCheckIntegrationlatestSuccessDate " + e.getMessage());
			}
			
		}
		if("RO_allQueueUpdate".equals(controlName))  // this is required to update RO code in all legs
		{
			try 
			{	
				String query="";
				String mainRO="";
				String ReassignRO="";
				mainRO=(String) iform.getValue("RO");
				ReassignRO=(String) iform.getValue("Q_RB_iRBL_TXNTABLE_Reassigned_RO");
				String strValues="";
				if(!"".equalsIgnoreCase(mainRO) && !"".equalsIgnoreCase(ReassignRO))
				{
					strValues="'"+mainRO+","+ReassignRO+"'";
				}
				else if(!"".equalsIgnoreCase(mainRO) && "".equalsIgnoreCase(ReassignRO))
				{
					strValues="'"+mainRO+"'";
				}
				else if("".equalsIgnoreCase(mainRO) && !"".equalsIgnoreCase(ReassignRO))
				{
					strValues="'"+ReassignRO+"'";
				}
		        query = "update WFINSTRUMENTTABLE set VAR_STR7 ="+strValues+" where ProcessInstanceID = '"+getWorkitemName(iform)+"'";
		...
		if("RM_allQueueUpdate".equals(controlName))  //  // this is required to update RM code in all legs
		{
			try 
			{	
				String query="";
				String mainRM="";
				String ReassignRM="";
				mainRM=(String) iform.getValue("RM");
				ReassignRM=(String) iform.getValue("Q_RB_iRBL_TXNTABLE_Reassigned_RM");
				String strValues="";
				if(!"".equalsIgnoreCase(mainRM) && !"".equalsIgnoreCase(ReassignRM))
				{
					strValues="'"+mainRM+","+ReassignRM+"'";
				}
				else if(!"".equalsIgnoreCase(mainRM) && "".equalsIgnoreCase(ReassignRM))
				{
					strValues="'"+mainRM+"'";
				}
				else if("".equalsIgnoreCase(mainRM) && !"".equalsIgnoreCase(ReassignRM))
				{
					strValues="'"+ReassignRM+"'";
				}
		        query = "update WFINSTRUMENTTABLE set VAR_STR18 ="+strValues+"  where ProcessInstanceID = '"+getWorkitemName(iform)+"'";
	        	iform.saveDataInDB(query);
			}
			catch (Exception e) 
			{
		...
		if("OPS_Decision".equals(controlName))  // if ops/au policy actioning the WI at the same time-handling
		{
			String OPS_decision = "";
			try 
			{				
				List lstDecisions = iform
					.getDataFromDB("select dec_system_check_checker from RB_iRBL_EXTTABLE with(nolock) where WINAME='"+getWorkitemName(iform)+"'");
				iRBL.mLogger.debug("WINAME : "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", lstDecisions : "+lstDecisions.toString());
								
				for(int i=0;i<lstDecisions.size();i++)
				{
					List<String> arr1=(List)lstDecisions.get(i);
					OPS_decision= arr1.get(0);
				}
				strReturn = OPS_decision;
